...
export default function SkillOptimizePage() {
    ...
    const startOptimize = async (opts?: { planId?: string; planSourceIssueIds?: string[]; feedbackText?: string }) => {
        ...
        try {
            ...
            // 本轮实际「已解决」的 issue id：plan 模式（一键优化）用 plan 的源 issue id；
            // 手动模式用后端 rank/select 收束后的本轮 issue（scopedIssueIds 过滤的 resolvedThisRound）。
            const resolvedIds = opts?.planId ? resolveIds : resolvedThisRound.map(i => i.id);
            ...
            if (skill && user && resolvedIds.length > 0) {
                apiFetch(
            ...
            const draft: OptimizationIteration = {
                ...
                summary: agentSummary || (resolvedIds.length > 0
                    ? `针对 ${resolvedIds.length} 个本轮优化点的优化结果（agent 未输出总结）`
                    : checked.length > 0
