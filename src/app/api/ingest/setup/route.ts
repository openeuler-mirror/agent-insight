...
function generateBashScript(
    ...
    const lines = [
        ...
        '        AGENT_INSIGHT_HOST="$host" node "./scripts/install-ras-client.js" --host "$host" --token "$token"',
        '        return $?',
        '    fi',
        '',
        ...
        '            AGENT_INSIGHT_HOST="$host" node "$pkg_tmp/x/scripts/install-ras-client.js" --host "$host" --token "$token"',
        '            rc=$?',
        '            rm -rf -- "$pkg_tmp"',
        '            return $rc',
        '        fi',
        ...
        '        AGENT_INSIGHT_HOST="$host" node "$pkg_tmp/x/package/scripts/install-ras-client.js" --host "$host" --token "$token"',
        '        rc=$?',
        '    else',
        ...
        'install_agent_insight_client "$FINAL_HOST" "$FINAL_KEY" || echo "⚠️  常驻客户端未注册；Trace 采集不受影响，但「客户端配置」页看不到本机。"',
        '',
