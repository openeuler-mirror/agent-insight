...
function generateBashScript(host: string, baseUrl: string, apiKey: string, preselected: { value: string; label: string }[], llamaIndexVenv: string, llamaIndexPythonMode: string): string {
    ...
    const lines = [
        ...
        'if [ "$INSTALL_OPENCODE" = "false" ] && [ "$INSTALL_CLAUDE" = "false" ] && [ "$INSTALL_OPENCLAW" = "false" ] && [ "$INSTALL_CODEAGENT" = "false" ] && [ "$INSTALL_HERMES" = "false" ] && [ "$INSTALL_JIUWEN" = "false" ] && [ "$INSTALL_LLAMAINDEX" = "false" ]; then',
        ...
        '    if [ "$LLAMAINDEX_CONFIGURED_MODE" = "auto" ] && { : < /dev/tty; } 2>/dev/null; then',
...
function generatePowerShellScript(host: string, baseUrl: string, apiKey: string, preselected: { value: string; label: string }[], llamaIndexVenv: string, llamaIndexPythonMode: string): string {
    const llamaIndexOnly = preselected.length === 1 && preselected[0].value === 'llamaindex';
    const lines = [
        '# =============================================================================',
        ...
        '',
        '# Exit if nothing selected',
        'if (-not $INSTALL_OPENCODE -and -not $INSTALL_CLAUDE -and -not $INSTALL_OPENCLAW -and -not $INSTALL_CODEAGENT -and -not $INSTALL_HERMES -and -not $INSTALL_JIUWEN -and -not $INSTALL_LLAMAINDEX) {',
        ...
        '    if ($llamaIndexConfiguredMode -eq "auto" -and $llamaIndexCanPrompt) {',
...
export async function GET(request: Request) {
    ...
    if (platform === 'windows') {
        const script = generatePowerShellScript(skillInsightHost, baseUrl, apiKey, preselected, llamaIndexVenv, llamaIndexPythonMode);
        ...
        const script = generateBashScript(skillInsightHost, baseUrl, apiKey, preselected, llamaIndexVenv, llamaIndexPythonMode);
        return new NextResponse(script, {
            headers: {
