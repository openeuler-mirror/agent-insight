...
'    { name: \'TRAE AI IDE\', value: \'trae\' }',
        ...
        'if [ "$INSTALL_OPENCODE" = "false" ] && [ "$INSTALL_CLAUDE" = "false" ] && [ "$INSTALL_HERMES" = "false" ] && [ "$INSTALL_JIUWEN" = "false" ] && [ "$INSTALL_TRAE" = "false" ]; then',
        ...
        '    rm -f "$OPENCODE_CONFIG_DIR/plugins/Skill-Insight.ts" "$OPENCODE_CONFIG_DIR/plugins/Witty-Skill-Insight.ts" "$OPENCODE_CONFIG_DIR/plugins/Witty-Skill-Insight.tui.tsx" 2>/dev/null || true',
        ...
        '    # TUI 插件是终端(TUI)专用的统计页脚，依赖 solid-js/@opentui/solid；',
        '    # 非交互(desktop/headless/共享账户)场景用不到，历史上还会因缺依赖拖垮 opencode 启动 → 跳过',
        '    if [ "$NONINTERACTIVE" = "true" ]; then',
        '      echo "⏭️  非交互模式：跳过 TUI 统计插件(仅终端 TUI 需要)。"',
        '    else',
...
'fi',
        ...
        'fi',
        '',
        '# -- Per-account isolation: hash the API key so opencode spool/checkpoint are',
        '#    namespaced per account. Switching accounts on the same machine no longer',
        '#    replays another account\'s history under the current key.',
        'CLIENT_KEY_HASH=$(printf \'%s\' "$FINAL_KEY" | { shasum -a 256 2>/dev/null || sha256sum; } | cut -c1-16)',
        'NOW_MS=$(node -e \'process.stdout.write(String(Date.now()))\' 2>/dev/null || echo $(( $(date +%s) * 1000 )))',
        '# Keep an existing baseline when re-installing the SAME key; reset to now for a',
        '# new/changed key so only traces produced after THIS install are uploaded.',
        'if [ -n "$EXISTING_UPLOAD_SINCE_MS" ] && [ "$FINAL_KEY" = "$EXISTING_KEY" ]; then',
        ...
        '    JW_OTLP_ENDPOINT="${FINAL_HOST%/}/api/ingest/otel/v1/traces"',
...
function generatePowerShellScript(host: string, baseUrl: string, apiKey: string, noninteractive: boolean = false, frameworks: string = 'opencode', nokey: boolean = false): string {
    const lines = [
        ...
        '',
        '# Exit if nothing selected',
        'if (-not $INSTALL_OPENCODE -and -not $INSTALL_CLAUDE -and -not $INSTALL_HERMES -and -not $INSTALL_JIUWEN -and -not $INSTALL_TRAE) {',
        ...
        '    Remove-Item -Path (Join-Path $opencodeConfigDir "plugins\\Witty-Skill-Insight.tui.tsx") -Force -ErrorAction SilentlyContinue',
        ...
        '    # TUI 插件是终端(TUI)专用统计页脚，依赖 solid-js/@opentui/solid；',
        '    # 非交互(desktop/headless/共享账户)用不到，历史上会因缺依赖拖垮 opencode 启动 → 跳过',
        '    if ($NONINTERACTIVE) {',
        '        Write-Host "⏭️  非交互模式：跳过 TUI 统计插件(仅终端 TUI 需要)。"',
        '    } else {',
        ...
        '    $sinceLine = $content | Select-String \'^AGENT_INSIGHT_OPENCODE_UPLOAD_SINCE_MS=\' | Select-Object -First 1',
        '    if ($keyLine) {',
        ...
        '    }',
        '    if ($sinceLine) {',
        '        $EXISTING_UPLOAD_SINCE_MS = $sinceLine.Line.Substring(\'AGENT_INSIGHT_OPENCODE_UPLOAD_SINCE_MS=\'.Length)',
        '    }',
        '}',
        '',
        '# -- API Key Logic --',
        ...
        '$CLIENT_KEY_HASH = ([System.BitConverter]::ToString(([System.Security.Cryptography.SHA256]::Create()).ComputeHash([System.Text.Encoding]::UTF8.GetBytes([string]$FINAL_KEY))) -replace \'-\',\'\').ToLower().Substring(0,16)',
        '$NOW_MS = [string][long]([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())',
        'if ($EXISTING_UPLOAD_SINCE_MS -and ($FINAL_KEY -eq $EXISTING_KEY)) { $UPLOAD_SINCE_MS = $EXISTING_UPLOAD_SINCE_MS } else { $UPLOAD_SINCE_MS = $NOW_MS }',
        '',
        ...
        '$envLines += "AGENT_INSIGHT_OPENCODE_SPOOL_DIR=$homeDir\\.agent-insight\\otel_data\\opencode\\$CLIENT_KEY_HASH"',
        '$envLines += "AGENT_INSIGHT_OPENCODE_CHECKPOINT=$homeDir\\.agent-insight\\opencode_uploader_checkpoint_$CLIENT_KEY_HASH.json"',
        '$envLines += "AGENT_INSIGHT_OPENCODE_UPLOAD_SINCE_MS=$UPLOAD_SINCE_MS"',
        '$envLines += "AGENT_INSIGHT_OPENCODE_UPLOADER=$homeDir\\.agent-insight\\opencode_uploader_client.js"',
        '$envLines += "AGENT_INSIGHT_CLAUDE_OTEL_SPOOL_DIR=$homeDir\\.agent-insight\\otel_data\\claude"',
        '$envLines += "AGENT_INSIGHT_CLAUDE_OTEL_RAW_API_BODIES=file:$homeDir\\.agent-insight\\claude_raw_bodies"',
        ...
        '    $hermesPluginDir = Join-Path $hermesHome "plugins\\agent_insight_hermes"',
        '    New-Item -ItemType Directory -Path $hermesPluginDir -Force | Out-Null',
        '    $hermesConfig = @{ host = $FINAL_HOST.TrimEnd("/"); api_key = $FINAL_KEY; service_name = "hermes"; max_content_chars = 200000; spool_dir = (Join-Path $skillInsightDir "data\\hermes-otel-spool"); log_file = (Join-Path $skillInsightDir "logs\\hermes-plugin.log") } | ConvertTo-Json',
        ...
        '    $jwOtlpEndpoint = $FINAL_HOST.TrimEnd("/") + "/api/ingest/otel/v1/traces"',
        '    New-Item -ItemType Directory -Path (Join-Path $jwHome "config") -Force | Out-Null',
