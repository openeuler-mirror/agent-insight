...
function generateBashScript(host: string, baseUrl: string, apiKey: string, preselected: { value: string; label: string }[]): string {
    const lines = [
        ...
        '    # OTel 事件里没有,靠这个 SessionEnd hook 在会话结束时补发。',
        ...
        '            node "$HOME/.agent-insight/claude_context_uploader.cjs" --install-hook || echo "⚠️  注册 SessionEnd hook 失败,可稍后手动执行:node $HOME/.agent-insight/claude_context_uploader.cjs --install-hook"',
        '        else',
