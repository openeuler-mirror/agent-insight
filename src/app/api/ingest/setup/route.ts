...
function generateBashScript(host: string, baseUrl: string, apiKey: string, preselected: { value: string; label: string }[]): string {
    const lines = [
        ...
        'if [ "$LLAMAINDEX_READY" = "true" ]; then echo "  6. Restart terminal, then run: $LLAMAINDEX_PYTHON -m agent_insight_llamaindex.cli run -- $LLAMAINDEX_PYTHON app.py"; fi',
        'echo "------------------------------------------------"',
    ];
    return lines.join('\n');
}
...
function generatePowerShellScript(host: string, baseUrl: string, apiKey: string, preselected: { value: string; label: string }[]): string {
    const lines = [
        ...
        'if ($LLAMAINDEX_READY) { Write-Host "  6. Restart PowerShell, then run: $llamaIndexPython -m agent_insight_llamaindex.cli run -- $llamaIndexPython app.py" }',
        'Write-Host "------------------------------------------------"',
    ];
    return lines.join('\n');
}
