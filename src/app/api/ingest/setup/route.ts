...
function generateBashScript(host: string, baseUrl: string, apiKey: string): string {
    const lines = [
        ...
        '    HERMES_OTEL_PLUGIN_DIR="$HERMES_HOME/plugins/hermes_otel"',
        '    mkdir -p "$HERMES_OTEL_PLUGIN_DIR"',
        '    HERMES_OTEL_ENDPOINT="$FINAL_HOST"',
        ...
        'if [ "$INSTALL_HERMES" = "true" ]; then echo "  ✅ Hermes OTel Plugin: ${HERMES_HOME:-$HOME/.hermes}/plugins/hermes_otel/config.yaml"; fi',
        '',
        'if [ "$NEEDS_WATCHER_SCRIPTS" = "true" ]; then',
        '    echo ""',
...
function generatePowerShellScript(host: string, baseUrl: string, apiKey: string): string {
    const lines = [
        ...
        '    }',
        '    $venvCandidates = @($env:HERMES_AGENT_VENV, $env:HERMES_VENV, (Join-Path $homeDir "git\\hermes-agent\\venv"), (Join-Path $homeDir "agent\\hermes-agent\\venv"))',
        ...
        'if ($INSTALL_HERMES) { $summaryHermesHome = if ($env:HERMES_HOME) { $env:HERMES_HOME } else { Join-Path $homeDir ".hermes" }; Write-Host "  ✅ Hermes OTel Plugin: $summaryHermesHome\\plugins\\hermes_otel\\config.yaml" }',
        '',
        'if ($NEEDS_WATCHER_SCRIPTS) {',
        '    Write-Host ""',
