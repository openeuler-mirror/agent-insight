...
export function sqlDatabase(client: any): SqlDatabase {
    const postgres = typeof client.connect === 'function';
    const wrap = (connection: any): SqlConnection => {
        ...
        return {
            async rows<T>(sql: string, values: unknown[] = []): Promise<T[]> {
                return postgres ? (await connection.query(statement(sql), values)).rows : connection.$queryRawUnsafe(sql, ...values);
            },
    ...
    return {
        ...
        async transaction<T>(work: (connection: SqlConnection) => Promise<T>): Promise<T> {
            if (!postgres) return client.$transaction((tx: any) => work(wrap(tx)), { maxWait: 10000, timeout: 15000 });
...
export type SavedEvent = { id: string; eventId: string; bodyJson: string; bodyHash: string; receivedAt: string };
...
export class CollaborationStore {
    ...
    private async ensure(connection: SqlConnection, user: string, id: string) {
        await connection.execute('INSERT INTO "Collaboration" ("id","user","collaborationId","createdAt") VALUES (?,?,?,?) ON CONFLICT ("user","collaborationId") DO NOTHING', [randomUUID(), user, id, new Date().toISOString()]);
    }
    async saveEvent(user: string, event: RelationEvent) {
        return this.sql.transaction(async tx => {
            await this.ensure(tx, user, event.collaborationId);
            const id = randomUUID();
            await tx.execute('INSERT INTO "CollaborationEvent" ("id","user","collaborationId","eventId","bodyJson","bodyHash","receivedAt") VALUES (?,?,?,?,?,?,?) ON CONFLICT ("user","collaborationId","eventId") DO NOTHING', [id, user, event.collaborationId, event.eventId, canonical(event), digest(event), new Date().toISOString()]);
            const [saved] = await tx.rows<SavedEvent>('SELECT "id","eventId","bodyJson","bodyHash","receivedAt" FROM "CollaborationEvent" WHERE "user"=? AND "collaborationId"=? AND "eventId"=?', [user, event.collaborationId, event.eventId]);
    ...
    async snapshot(user: string, id: string, offset: number, limit: number) {
        ...
        return this.sql.transaction(async tx => {
            ...
            const page = await tx.rows<SavedEvent>('SELECT "id","eventId","bodyJson","bodyHash","receivedAt" FROM "CollaborationEvent" WHERE "user"=? AND "collaborationId"=? ORDER BY "receivedAt","id" LIMIT ? OFFSET ?', [user, id, limit, offset]);
            const events = total <= MAX_GROUP_EVENTS ? await tx.rows<SavedEvent>('SELECT "id","eventId","bodyJson","bodyHash","receivedAt" FROM "CollaborationEvent" WHERE "user"=? AND "collaborationId"=? ORDER BY "receivedAt","id" LIMIT ?', [user, id, MAX_GROUP_EVENTS + 1]) : [];
