...
const GLOSSARY = {
  ...
  'traj-completeness': {
    name: '完整性',
    tag: 'eval',
    body: '评测 Agent 是否覆盖了该 Skill 要求的关键步骤、顺序是否正确——即"该做的有没有做、有没有漏步/乱序"。trace 模式下取 analyze-match 确定性对齐的覆盖率。',
    formula: '覆盖率(命中关键步骤 / 应有关键步骤) − 顺序惩罚 · 轨迹分权重 45%',
  },
  'traj-tool-choice': {
    name: '工具选择',
    tag: 'eval',
    body: '评测每一步是否选对工具 / Skill、参数与调用时机是否合理——即"工具用得对不对、巧不巧"。',
    formula: '逐步打分均值（选对 1.0 / 时机不当 0.7 / 参数错 0.5 / 选错 0）· 轨迹分权重 35%',
  },
  'traj-redundancy': {
    name: '冗余',
    tag: 'eval',
    body: '评测是否存在重复调用、绕路、死循环式的多余操作——即"有没有废动作、执行效率高不高"。由规则统计连续重复 / 高频调用得出，不靠 LLM 数循环。',
    formula: '1 − (连续重复段 × 0.2 + 高频调用 × 0.1) · 轨迹分权重 20%',
  },
  'one-line-diagnosis': {
    name: '一句话诊断',
    tag: 'fault',
  ...
  'quality-dim-process': {
    name: '过程维（路径对不对）',
    tag: 'metric',
    body: '有效子指标的均值，识别"蒙对"（结果好/过程差）与"白忙"（过程好/结果差）。\n工具正确性：1 − 工具错误次数 ÷ 调用次数（恒可算）\n计划效率 / 约束遵循 / 工具归因：依赖轨迹评测，缺数据记 N/A 不入分母\n覆盖率低 = 只测了工具正确性一个角度，分数偏"薄"',
    formula: '过程 = mean(非 N/A 子指标)\n基准线：达标 ≥85 · 关注 70–85 · 异常 <70',
  },
  'quality-dim-cost': {
    name: '成本维（划不划算）',
    tag: 'metric',
    body: '每条 trace 的"预算余量分"求均值：取 时延/Token/步数 三者中最紧张的占用率，剩余空间即得分。预算上限为可配置参数（默认 120 秒 / 20 万 token / 60 步，待标定）。\n注意：均值会掩盖失控长尾，须结合趋势里的 p95 一起看。',
    formula: '单条成本分 = (1 − max(时延/上限, token/上限, 步数/上限)) × 100\n基准线：达标 ≥85 · 关注 70–85 · 异常 <70',
  },
  'quality-dim-error': {
    name: '错误维（在哪儿崩）',
    tag: 'metric',
    body: '无错 trace 的占比（卡片大数字显示的是报错次数，状态按错误分判定）。\n错误事件来自重解析执行步骤：工具步看返回内容，LLM 步只看结构化元信息——模型"谈论失败"不算"步骤失败"。',
