...
export function buildSkillOptSystemPrompt(args: BuildSkillOptPromptArgs): string {
  const { skillName, baseVersion, checkedIssues, userFeedback, optimizationScope } = args;
  ...
  const issuesSection = formatIssuesSection(
    sortedIssues,
  ...
  return [
    ...
    '6. **Gate**：如果某个本轮 issue 证据不足、建议不可执行，或需要额外 trace 才能判断，不要扩大改写范围；把它放入"暂未处理"并说明需要什么信息。',
    '',
    '# 运行环境',
    '',
