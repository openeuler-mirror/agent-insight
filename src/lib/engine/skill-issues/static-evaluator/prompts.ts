...
### 评分前必须先判定任务风险档位（扣分要按风险加权）
- 高风险：会修改系统状态/数据、执行不可逆或对外操作（删除、转账、改配置、远程执行、登录远程主机等）。
- 低风险：只读 / 离线 / 纯分析，且 SKILL.md 明确声明「不执行、不连远程、仅给建议」——出错后果有限。
...
- 5 分：自由度与任务风险完全匹配。
- 3 分：基本匹配，少量场景过严或过松。
- 1-2 分：**仅用于高风险方向的严重错配**——高风险/不可逆操作却让 LLM 自由判断（欠约束，可能造成不可逆后果）。
- 对「低风险任务过度刚性」（如只读/离线分析却强制无脑全跑、禁止按上下文跳过明显无关的步骤）：这是效率/适配问题，不是安全错配，**最低只扣到 3 分，不得给 1-2 分**；只有当刚性已严重妨碍任务完成时才可更低。
