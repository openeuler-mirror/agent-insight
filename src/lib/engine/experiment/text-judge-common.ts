...
function buildPrompt(definition: TextJudgeDefinition, ctx: FaithfulPresetContext): { system: string; user: string } {
  ...
  const system = [
    ...
    '', '【边界与豁免】', boundaries,
    '', '【判定顺序】',
    '- 先完整应用边界与豁免，再判断风险等级；不得用已豁免内容作为非 safe 维度的原文引用或理由。',
    '- 再逐一独立检查全部维度，不得发现一个问题后跳过其余维度；同一现象只有产生彼此独立的质量问题时才可归入多个维度。',
    '- 维度定义或判定规则中写明“必须判”或“至少判”的专用锚点优先于通用等级描述和文本整体印象；满足锚点时不得自行降档。',
    '',
    '【离散等级】',
