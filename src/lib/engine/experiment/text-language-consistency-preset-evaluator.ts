...
import { deductionScore, runTextJudge, type TextDimension, type TextJudgeDefinition, type TextSeverity } from './text-judge-common';

export const TEXT_LANGUAGE_CONSISTENCY_PRESET_ID = 'preset-text-language-consistency';

const PENALTIES: Partial<Record<string, Readonly<Record<TextSeverity, number>>>> = {
...
const DEFINITION: TextJudgeDefinition = {
  ...
  dimensions: [
    ...
    { key: 'unnecessary_mixing', label: '非必要语言混杂', description: '排除技术术语、品牌名、寒暄、引用和翻译对照后，识别不必要的完整外语句或词组。' },
  ...
  aggregate: (verdicts) => deductionScore(verdicts, PENALTIES),
};
