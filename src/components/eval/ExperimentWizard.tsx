...
export function ExperimentWizard({
  ...
  const filteredGenerationCases = useMemo(() => {
  ...
  const selectWizardDataset = useCallback(async (nextId: string): Promise<DatasetOption | null> => {
    ...
    try {
      ...
      if (datasetRequestIdRef.current !== requestId) return null;
  ...
  useEffect(() => {
    ...
    const loadInitialConfig = async () => {
      if (!reuseFrom) {
        if (wizardDatasets.some((dataset) => dataset.id === requestedDatasetId)) {
          await selectWizardDataset(requestedDatasetId);
        }
        return;
      }
      ...
      const restoredEvaluators = Array.isArray(config.evaluatorIds)
        ...
        : [];
      const restoredCaseIds = Array.isArray(config.datasetCaseIds)
        ? new Set(config.datasetCaseIds.map(String).filter(Boolean))
        : new Set<string>();
      ...
      if (restoredDatasetId && wizardDatasets.some((dataset) => dataset.id === restoredDatasetId)) {
        ...
        if (restoredDataset && restoredCaseIds.size > 0) {
          ...
          if (restoredCases.length > 0) {
            setSelectedGenerated(new Map(restoredCases.map((item) => [item.executionId, item])));
          }
        }
        setSelectedEvaluators(new Set(restoredEvaluators));
      }
  ...
  useEffect(() => {
    if (!user || !agentName || expType !== 'llm') return;
    let cancelled = false;
    apiFetch(`/api/experiments/traces?agent=${encodeURIComponent(agentName)}&user=${encodeURIComponent(user)}&pageSize=100`)
      ...
      .then((d: { items?: Array<{ model?: string | null }> }) => {
        ...
        if (!cancelled) setAgentModels(models);
      })
      .catch(() => {
    ...
    try {
      ...
      const res = await apiFetch('/api/experiments', {
        ...
        body: JSON.stringify({
          ...
          datasetCaseIds: traceMode === 'generate' ? Array.from(selectedGenerated.keys()) : undefined,
          traceSource: traceMode,
          ...
          ...(!skillContext ? {
            configSnapshot: {
              ...
              type: expType,
              groups: expType === 'llm'
                ? [{ key: 'A', value: groupAValue.trim() }, { key: 'B', value: groupBValue.trim() }]
                : undefined,
              executionTarget: traceMode === 'generate' && selectedTarget ? {
                workerId: selectedTarget.workerId,
                platform: selectedTarget.platform,
      ...
      const runBody = isBenchmarkDataset
        ? {}
        : expType === 'single' && traceMode === 'generate' && selectedTarget
        ? {
            fiOrchestrate: isReliabilityDataset,
        ...
        {step === 1 && (
          <div style={PANEL}>
            <div style={PANEL_B}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                {skillContext ? (
                  ...
                  <>
                    ...
                    <button
                      type="button"
                      disabled={isBenchmarkDataset}
                      title={isBenchmarkDataset ? 'Benchmark 数据集仅支持单组实验' : undefined}
                      onClick={() => {
                      ...
                      style={{
                        ...
                        cursor: isBenchmarkDataset ? 'not-allowed' : 'pointer',
                        opacity: isBenchmarkDataset ? 0.45 : 1,
                      }}
                    >🔄 LLM 对比</button>
              ...
              {!skillContext && expType === 'llm' && groupAValue.trim() !== '' && groupBValue.trim() !== '' && groupAValue.trim() === groupBValue.trim() && (
            ...
            <div style={PANEL_B}>
              ...
              <div style={{ display: 'inline-flex', gap: 4, padding: 4, border: '1px solid var(--border)', borderRadius: 10, marginBottom: 14 }}>
                <button
                  type="button"
                  disabled={isBenchmarkDataset}
                  title={isBenchmarkDataset ? 'Benchmark 数据集必须生成新 Trace' : undefined}
                  style={{ ...BTN_GHOST, height: 30, opacity: isBenchmarkDataset ? 0.45 : 1, cursor: isBenchmarkDataset ? 'not-allowed' : 'pointer' }}
                  onClick={() => { if (!isBenchmarkDataset) setTraceMode('existing'); }}
                >选择 Trace</button>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 11.5, color: 'var(--foreground-muted)' }}>
                  已选择 {selectedGenerated.size} / {generationCases.length}
                </span>
                <button
                  type="button"
                  style={BTN_GHOST}
              ...
              {isBenchmarkDataset && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  ...
                  {benchmarkInstanceSearch.trim() && (
                    <span style={{ fontSize: 11.5, color: 'var(--foreground-muted)' }}>
                      找到 {filteredGenerationCases.length} 条
                    </span>
                  )}
                </div>
              )}
                            ...
                            <tr
                              ...
                              onClick={() => {
                                setSelectedGenerated((previous) => {
                                  ...
                                  if (next.has(item.executionId)) next.delete(item.executionId);
                                  else next.set(item.executionId, item);
                                  return next;
                                });
                              }}
                              style={{ cursor: 'pointer', background: checked ? 'var(--primary-subtle)' : 'transparent' }}
                            >
                ...
                {allEvaluators.map((card) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isBenchmarkDataset && !gate.usable && gate.reason && (
                        <div style={{ marginBottom: 8, padding: '6px 10px', borderRadius: 6, background: 'var(--warning-subtle)', border: '1px solid var(--warning-subtle-border)', color: 'var(--warning)', fontSize: 11 }}>
                          {gate.reason}
                        </div>
                      )}
