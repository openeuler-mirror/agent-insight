...
import {Stepper,ExpectedAnswersTable,GenerationCaseRow,ExperimentTypeSelector,experimentTableHeader as TH,experimentTableCell as TD} from '@/components/experiments/ExperimentWizard';
import AssetVersionPicker,{assetsFor,dimensionLabels,type AssetDimension} from './AssetVersionPicker';
...
import { ExperimentWizardPanel, ExperimentWizardHeading, ExperimentWizardFooter, experimentFieldLabelStyle as FIELDLBL, experimentInputStyle as INPUT, experimentGhostButtonStyle as BUTTON } from '@/components/experiments/ExperimentWizardLayout';
...
function DemoExperimentWizardContent(){
 ...
 function cases(side:'A'|'B'){
  ...
  return <section className="space-y-3" aria-label={side+' 组 Case'}>
   <div className="flex flex-wrap items-center justify-between gap-2">
    <h3 className="text-xs font-bold">{dimension==='dataset'?side+' 组 · ':''}{data?.name} · v{data?.version} <span className="font-normal text-foreground-muted">已选 {ids.length} / {data?.content.cases.length||0}</span></h3>
    <div className="flex gap-2"><button style={BUTTON} onClick={()=>setIds([...new Set([...ids,...rows.map(c=>c.id)])])}>全选</button><button style={BUTTON} onClick={()=>setIds([])}>取消全选</button></div>
   </div>
   <div style={{ border:'1px solid var(--border)', borderRadius:10, overflowY:'auto', maxHeight:360 }}>
    {rows.length===0?<p className="p-4 text-xs text-foreground-muted">{query?'未找到匹配的 Case':'评测集暂无 Case'}</p>:rows.map(c=><GenerationCaseRow key={c.id} checked={ids.includes(c.id)} label={side+' 组 Case '+c.name} onChange={()=>setIds(old=>old.includes(c.id)?old.filter(id=>id!==c.id):[...old,c.id])} action={<CaseRulesDialog value={c}/>}>
     <div className="flex items-center gap-2 text-xs font-bold"><span>{c.name}</span><span className="rounded bg-background-secondary px-2 py-0.5 text-[10px] font-normal text-foreground-muted">{c.turns.length>1?'多轮':'单轮'}</span></div>
     <div className="mt-1 text-[11px] text-foreground-muted">{c.turns[0].input}</div>
    </GenerationCaseRow>)}
   </div>
  </section>;
 }
 ...
 return <div className="min-w-0 text-[12.5px]"><ExperimentWizardHeading onBack={()=>router.push('/experiments')}/><Stepper labels={['实验设计','选择 Case','预期答案','确认并执行']} step={step} maxVisited={invalid?1:casesValid?4:2} summaries={[name,caseIds.length+' 个 Case',dataset?.name||'',dimension==='single'?'单组实验':dimensionLabels[dimension]+'对比']} optionalThird={false} onJump={setStep}/>
  {error&&<p role="alert" className="rounded-lg bg-error-subtle p-3 text-error">{error}</p>}
  <ExperimentWizardPanel><div className="space-y-4">
   {step===1&&<><label className="block"><span style={FIELDLBL}>实验名称</span><input aria-label="实验名称" style={INPUT} value={name} onChange={e=>setName(e.target.value)}/></label><fieldset><legend style={FIELDLBL}>实验类型</legend><ExperimentTypeSelector value={dimension} onChange={value=>{if(value!=='llm')setType(value);}} groupA="" groupB="" onGroupA={()=>{}} onGroupB={()=>{}} hideGroups allowDataset availableTypes={['single',...dimensions]}/></fieldset>
   {dimension!=='single'&&<section aria-label="对比项" className="space-y-3"><h3 className="text-xs font-bold">对比项 · {dimensionLabels[dimension]}</h3><div className="grid gap-3 md:grid-cols-2">{(['A','B'] as const).map(side=><div key={side} className="min-w-0 rounded-lg border border-border p-3 space-y-3"><h4 className="text-xs font-bold">{side} 组</h4>{picker(dimension,side)}</div>)}</div></section>}
   <section aria-label={dimension==='single'?'实验对象':'共享条件'} className="space-y-4">{dimension!=='single'&&<h3 className="text-xs font-bold">共享条件</h3>}<div className="grid gap-3 lg:grid-cols-2">{dimensions.filter(kind=>kind!==dimension).map(kind=><div key={kind}>{picker(kind)}</div>)}</div></section>
   <section aria-label="执行位置与模型" className="space-y-3 border-t border-border pt-3"><h3 className="text-xs font-bold">在哪里执行</h3><div className="grid gap-3 md:grid-cols-2"><label><span style={FIELDLBL}>执行地址 / IP</span><select aria-label="执行地址 / IP" style={INPUT} value={endpoint} onChange={e=>{setEndpoint(e.target.value);setModel('');}}><option value="">请选择执行地址</option>{endpoints.map(value=><option key={value} value={value}>{value}{value===catalog.executionOptions.demoEndpoint?'（独立 Demo Agent）':''}</option>)}</select></label><label><span style={FIELDLBL}>Agent 执行模型</span><select aria-label="Agent 执行模型" style={INPUT} value={model} onChange={e=>setModel(e.target.value)}><option value="">沿用 Agent 默认模型</option>{models.map(value=><option key={value} value={value}>{value}{value.startsWith('demo-')?'（规则模拟）':''}</option>)}</select></label></div><p className="text-xs text-foreground-muted">平台把所选 Case 发到这个地址执行，并收集实际输出、工具调用和 Trace。模型是传给 Agent 的执行参数，评估器的评分模型在评估器页面配置。</p></section>
   {invalid&&<p role="status" className="text-sm text-error">{invalid}</p>}
   </>}
   {step===2&&<><input style={INPUT} aria-label="搜索 Case" placeholder="搜索 Case 名称、输入或标签" value={query} onChange={e=>setQuery(e.target.value)}/>{cases('A')}{dimension==='dataset'&&cases('B')}</>}
   {step===3&&<><div className="flex items-center justify-between"><h3 className="text-xs font-bold">预期答案与逐轮规则</h3><Link style={BUTTON} href={'/dataset/versioned-'+a.dataset}>到评测集编辑</Link></div>{answers('A')}{dimension==='dataset'&&answers('B')}</>}
   {step===4&&<><h3 className="text-xs font-bold">确认实验条件</h3><div className="overflow-auto"><table className="w-full text-left text-sm"><thead><tr>{['对象','A 组 / 共享条件',...(dimension==='single'?[]:['B 组'])].map(label=><th key={label} style={TH}>{label}</th>)}</tr></thead><tbody>{dimensions.map(kind=>{const display=(side:'A'|'B')=>{const ids=kind==='evaluator'?(side==='A'?evalA:evalB):[(side==='A'?a:b)[kind]];return ids.map(id=>{const x=find(id);return x?x.name+' · v'+x.version:'沿用 Agent 内置';}).join('、');};return <tr key={kind}><td style={TD}>{dimensionLabels[kind]}</td><td style={TD}>{display('A')}</td>{dimension!=='single'&&<td style={TD}>{dimension===kind?display('B'):'与 A 组相同'}</td>}</tr>;})}</tbody></table></div><p>执行地址：{endpoint}<br/>Agent 模型：{model||'沿用 Agent 默认模型'}<br/>Case：{caseIds.length} 条{dimension==='dataset'?' / B 组 '+caseBIds.length+' 条':''}</p><label className="block max-w-xs"><span style={FIELDLBL}>通过率门槛（%）</span><input type="number" min="0" max="100" aria-label="通过率门槛" style={INPUT} value={threshold} onChange={e=>setThreshold(Number(e.target.value))}/></label><details><summary className="cursor-pointer text-sm">并发、超时与重试</summary><div className="mt-3 grid gap-3 md:grid-cols-3">{[{label:'并发数',value:concurrency,set:setConcurrency,min:1,max:8},{label:'超时（秒）',value:timeout,set:setTimeoutSeconds,min:1,max:600},{label:'重试次数',value:retries,set:setRetries,min:0,max:3}].map(item=><label key={item.label}><span style={FIELDLBL}>{item.label}</span><input type="number" aria-label={item.label} style={INPUT} min={item.min} max={item.max} value={item.value} onChange={e=>item.set(Number(e.target.value))}/></label>)}</div></details>{dimension==='evaluator'&&<p className="text-sm text-foreground-muted">Agent 只执行一次；A/B 评估器对同一份实际 Trace 分别评分。</p>}</>}
  </div><ExperimentWizardFooter step={step} busy={busy} onBack={()=>setStep(step-1)} onNext={step<4?()=>setStep(step+1):run} nextDisabled={!!invalid||(step>=2&&!casesValid)} nextLabel={step<4?['下一步：选择 Case →','下一步：预期答案 →','下一步：确认并执行 →'][step-1]:busy?'正在创建并启动…':'开始实验'}/>
  </ExperimentWizardPanel>
 </div>;
}
