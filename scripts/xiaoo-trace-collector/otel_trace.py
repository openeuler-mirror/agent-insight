# coding: utf-8
"""xiaoO session OTel buffer — Insight-owned (disk-backed across hooker spawns).

Persist under ``~/.agent-insight/xiaoo-otel-buf/<native_id>.json``.
"""
...
def _buf_path(native_id: str) -> Path:
    return _buf_root() / f"{native_id}.json"
...
def flush_session(session_id: str) -> bool:
    ...
    Skip POST when there are no llm/tool turns (avoids empty agent-root pollution).
    """
    native = strip_platform_prefix(session_id)
    with _LOCK:
        buf = _BUFFERS.get(native)
    ...
    if buf is None or not buf.has_content():
        ...
        return False
    if not buf.has_trace_spans():
        # User-only buffer: drop without POST (no meaningful ⓪ tree).
        logger.debug("otel flush skipped: no llm/tool turns for %s", native)
        with _LOCK:
            _BUFFERS.pop(native, None)
        _drop_persisted(native)
        return False
