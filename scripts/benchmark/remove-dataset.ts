...
function datasetIdFrom(args: string[]): string {
  ...
  if (!datasetId) {
    throw new Error('用法：npx tsx scripts/benchmark/remove-dataset.ts --dataset <dataset-id>')
  }
  return datasetId
}
...
async function main(): Promise<void> {
  ...
  const [{ removeSystemBenchmarkDataset }, { prisma }] = await Promise.all([
    import('../../src/lib/benchmark/dataset-admin-service'),
    import('../../src/lib/storage/prisma'),
  ])
  try {
    const result = await removeSystemBenchmarkDataset(datasetId)
