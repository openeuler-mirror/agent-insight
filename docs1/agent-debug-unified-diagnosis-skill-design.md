...
专项诊断器完成检测并经过通用富化后，结果交回现有 AgentDebug 流程，由 AgentDebug 在生成最终报告前完成：
...
运行公共 Trace 提取、静态分析和证据查询脚本
  ↓
运行现有 AgentDebug 五模块诊断
  ↓
扫描 Skill 内全部已启用专项诊断器
  ↓
对满足适用条件的诊断器执行检测
  ↓
通用富化器统一整理专项 finding
  ↓
把专项 finding 交回 AgentDebug
  ↓
AgentDebug 做语义查重和关联
  ↓
输出最终报告
...
- 可以组织到同一 cascadingChain；
- 或保留两个 finding，并在自然语言中说明关系；
- 不把不同机制强行合并为一个问题。
