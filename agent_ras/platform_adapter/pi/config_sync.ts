/**
 * pi-side helpers: pull Insight capability config and merge into local ras config.json.
 * Port of ../opencode/config_sync.js (fail-open — never throw into the extension).
 *
 * Local layout (shared file, per-platform slices):
 *   agent_ras.platforms.pi                # enabled / detectors / recovery
 *   agent_ras.platforms.pi.syncedFrom     # Insight provenance (not a decision cursor)
 */
...
function copyDetectorMap(detectors: any) {
...
function capabilitySlice(body: any) {
...
export function capabilityFingerprint(body: any): string {
  if (!body || typeof body !== "object") return ""
  return stableStringify(capabilitySlice(body))
}

export function capabilityContentHash(body: any): string {
  const fp = capabilityFingerprint(body)
  if (!fp) return ""
  return createHash("sha256").update(fp).digest("hex").slice(0, 16)
}

export function resolvePlatformCapabilityFromRas(ras: any, platform: string) {
...
export function mergeCapabilityIntoLocalRasConfig(
  ...
  const syncedFrom: Record<string, any> = { contentHash }
  ...
  const nextRas: Record<string, any> = {
    ...prevRas,
...
function isLoopbackUrl(urlStr: string) {
  try {
...
/** Loopback ras-config curl connect timeout (seconds). Keep short: runs on pi extension load. */
...
export async function defaultRasConfigFetch(url: string, init: any = {}) {
...
export function resolveRasConfigUrl(insight: any) {
  const fromEnv =
...
export async function syncCapabilityConfigFromInsight(opts: any = {}) {
  const log = opts.log || (() => {})
  const fetchImpl = opts.fetchImpl || defaultRasConfigFetch
  const rasHome = opts.rasHome || resolveRasHome()
  const platform = opts.platform || "pi"
