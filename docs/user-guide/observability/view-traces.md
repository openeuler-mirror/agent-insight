...
版本标签和业务标签在「配置 / 版本管理」中维护。两类标签都可用于链路追踪列表筛选，版本标签还用于版本分析的分组维度。管理页分别按标签名称中首个 `_` 或 `-` 之前的前缀聚类为卡片，不含分隔符的标签进入“未分组”；卡片汇总标签数与 Trace 绑定数，并支持预填同前缀快速新建。两类标签都可以从 Trace 列表的用户标签列绑定到具体 Trace。编辑标签会同步影响链路追踪、版本分析和用户标签筛选中的展示，不会丢失已绑定 Trace；删除标签是硬删除，会移除对应 Trace 绑定，其中版本标签还会影响版本分析。
