...
上图展示了链路追踪列表页的典型结构：顶部是聚合指标，指标下方是筛选区，中间是执行记录表格，右侧操作列用于进入详情页。830 转测版不在链路追踪中提供直接分析或运行评测的入口；已支持的「加入评测集」数据收集操作继续保留。
...
这一区域更适合回答“最近是否整体变慢”“工具调用是否异常”“问题是否集中在某一时间段”。
...
- **子 Agent 类型**：在“过滤”中选择当前用户已有子任务的类型；切换范围为“仅子 Agent”或“主 Agent + 子 Agent”后，可筛出对应子任务
...
- **Trace ID**：当前执行的唯一标识。列宽不足时末尾省略，拖宽列后可显示完整 ID；悬停查看和复制始终使用完整值。
...
其中，**执行状态**表示 Trace 生命周期：有明确完成信号时显示成功；AcTrail 根进程有效非零退出码显示失败。所有 Agent、所有采集框架（包括未知框架），未明确结束且连续 **10 分钟没有采集更新**时显示“超时”，无需已有回答。新数据以服务端采集更新入库时间重新计时；历史记录没有该字段时使用记录时间兼容判断。超时不是明确的执行失败，不伪造结束时间，也不因此触发评测；后续新信息到达可恢复执行中。列表和详情每 5 秒更新状态，详情超时后继续接收恢复信息；明确结束后停止自动刷新。暂停详情自动刷新时可点击立即刷新。

可将列表页理解为一次“样本筛选台”：先判断哪些 Trace 值得看，再进入详情做证据级分析。

Agent、执行状态、Tokens、执行时间表头统一显示上下排序箭头。未排序列的两枚箭头均为灰色，当前排序列高亮对应方向；上箭头表示升序，下箭头表示降序。默认按执行时间降序，点击表头切换排序，刷新后保留网址中的排序条件。
...
版本标签和业务标签在「配置 / 版本管理」中维护。版本标签用于版本分析的分组维度；业务标签用于标记 Trace 的业务分类。管理页分别按标签名称中首个 `_` 或 `-` 之前的前缀聚类为卡片，不含分隔符的标签进入“未分组”；卡片汇总标签数与 Trace 绑定数，并支持预填同前缀快速新建。两类标签都可以从 Trace 列表的用户标签列绑定到具体 Trace。编辑标签会同步影响链路追踪和版本分析中的展示，不会丢失已绑定 Trace；删除标签是硬删除，会移除对应 Trace 绑定，其中版本标签会影响版本分析，业务标签会移除对应分类标记。
...
- **加入评测集**：将单条或批量选择的 Trace 收集到评测数据集；后续评测从评测中心创建实验。
- **导出 Trace**：在详情页下载完整 Trace Bundle。

链路追踪不提供直接分析、运行评测或 Infra 关联入口；筛选区与搜索字段不展示成本、答案分、答案正确及 Skill 评分指标。旧链接中的这些筛选条件也不参与当前页面查询。详情摘要保留执行状态、耗时和 Token。
...
上图展示了详情页的典型结构：顶部是 Trace 摘要与统计指标，中部按标签页聚合不同调用维度，左侧是 Span 列表，中央是时间轴，右侧是当前选中节点的摘要信息。AcTrail 采集的 Trace 顶部标签显示实际 Agent 名称（如 OpenCode），悬停提示“采集来源：AcTrail”；缺少 Agent 名称时回退为 AcTrail。列表中的框架筛选及系统标签仍表示采集来源。

在 Agent 的“时间线”中点击工具事件摘要或“查看”，可打开事件详情弹窗。Input 和 Output 内容区顶部均有 **Copy** 按钮，用于复制各自的完整内容；JSON 输入支持复制整个对象，文本输出也可以直接复制。
...
点击节点输入或输出区的“查看全部”，弹窗会默认展开全部 JSON 层级和数组项，并完整显示长字符串；内容较长时可滚动查看，无需逐层点击省略号。右上角 **Copy** 用于复制完整原文，支持通过内网 HTTP 地址访问时复制；复制后可粘贴到文本编辑器核对内容。侧栏预览仍保留原有折叠方式。
...
AcTrail Trace 的 LLM Input 同样直接使用每次模型请求上报的完整消息，而不是从相邻事件猜测。内容块会先整理成可读文本，系统提示和旧轮次放入 History；本轮工具交互按调用标识显示为一组 Assistant 工具调用与 User 工具结果，不会因为旧消息的正文发生轻微变化而把多轮历史混入 Current input。单条消息及历史条数仍有上限保护。AcTrail 没有单独上报某个工具结果时，平台会从后续模型请求中按相同工具调用标识回填结果，并将其后连续的文本块一起作为输出。失败的模型调用会显示 AcTrail 实际上报的 HTTP 状态码和原因，例如 `LLM 调用失败：HTTP 404 Not Found`。Agent 名称优先使用 AcTrail 明确上报的名称，其次使用根进程 `agent.identity` 上的 `actrail.agent.type`：例如 `opencode` 显示为 `OpenCode`，`claude-code` 显示为 `Claude Code`，`xiaoo` 等其他产品名按上报值显示。该字段是 AcTrail 推断结果；来源为 `executable` 时可能仅是 `python3` 等程序名。字段缺失时仍兼容请求头及旧版命名信息，忽略 `agent.identity`、`agent identity process-*` 通用占位名，最终回退为 `AcTrail Agent`。采集来源仍为 AcTrail；身份信息晚到后会更新名称，历史记录需重新处理保留的采集数据才能更新。

Langfuse 按已结束 span 增量上报时，子 Agent 可能早于应用根 span 到达。平台会等待可确认的顶层 span 后再生成主 Trace，避免把 `intent-agent` 等子 Agent 临时显示为主 Agent。

### 调用树展开与慢节点筛选

- 工具栏的“展开 / 收起”保持原有行为：控制子 Agent 及下层可展开节点，收起时保留根 Agent 展开及其直接节点。按钮旁带有与 TRACE ID 表头一致的信息图标，悬停或键盘聚焦时弹出同样的说明卡片；没有子 Agent 或可折叠层级时，点击收起可能看不到列表变化。
- “仅慢节点（>60s）”按耗时严格大于 60 秒筛选事件，不表示错误或其他异常。Agent 行保留作层级上下文；按钮旁数字沿用慢 Agent 数量。
- 开启“仅慢节点”后，耗时选项显示 `>60s` 并暂时不可修改；关闭后恢复此前选择。需要改为 `>5s` 等条件时，先关闭“仅慢节点”。普通耗时选项也支持 `>60s`；恰好 60 秒与耗时未知的事件不命中该档。
- 类型、Token 和搜索条件仍与慢节点条件共同生效；“重置”清除所有树筛选条件。
...
子 Agent 行或卡片的 **Trace** 按钮可打开独立详情；点击“父执行”返回后，可以再次打开同一子 Trace。父子切换会更新详情，不需要刷新页面。
...
LLM 的 Input/Output 消息中，Thought、Response 和 Tool calls 的箭头向右表示收起，向下表示展开，与外层区块一致；点击各区块可独立展开或收起。
...
830 链路追踪页的“重置筛选”紧跟筛选控件，用于恢复当前列表筛选。点击记录行进入 Trace 详情页。业务标签仍可管理和打标，但此页不提供业务标签筛选，旧链接的 `bizTag` 参数也不再影响列表查询。
