...
**环内 RAS（可靠性）与普通链路追踪解耦。** 「运行观测 / 链路追踪」（`/trace`）列表与详情**不再**展示 RAS 徽章或异常面板；环内异常与恢复请到侧栏 **AgentRAS 可靠性 / 可靠性观测**（`/agent-ras/trace`）查看。该页以当前账号的普通根 Trace 为主表，左连接 RAS 事件，同时包含无故障 Trace 和异常 Trace。详情页在异常卡之外复用完整链路视图，并叠 RAS 标记。没有对应 Execution 的合成测试事件不会作为 Trace 展示。
