...
- **执行状态**：成功、失败、运行中等状态结果；若该次执行在环内触发过 Agent RAS，状态列下方会显示环内异常种类徽章（如「工具重复调用」）
...
**环内 RAS 标识**与执行状态正交：它表示宿主在运行中检测到的异常与恢复动作，按 `taskId` 关联到对应 Trace。普通链路详情展示摘要徽章；「运行观测 / 可靠性观测」则以当前账号的普通根 Trace 为主表，左连接 RAS 事件，因而同时包含无故障 Trace 和异常 Trace。没有对应 Execution 的合成测试事件不会作为 Trace 展示。

可靠性观测列表额外提供 **RAS 处置**列，按「有故障 → 是否启动恢复 → 恢复是否成功」展示，与 **执行状态** 解耦：

- **执行状态**只看 session 是否结束、是否有 `finalResult`、是否有 recorded `failures`：执行中 / 正常完成 / 执行失败。RAS 下发的 `abort_stream` **不会**单独把执行状态标成失败——该动作是恢复手段（打断坏输出流），恢复成功且任务正常收尾时应显示「正常完成」。
...
在「AgentRAS 可靠性 / 可靠性观测」顶栏可进入 **故障模式** 目录页（`/agent-ras/fault-modes`，侧栏亦有入口）。该页按「故障模式 → 子故障模式 → 恢复措施 → 严重度」列出当前 runtime **已落地** 的细粒度检测能力（如输出崩溃-字面精确循环、逻辑死循环、输出崩溃-规划执行死锁、ping_pong），粒度细于观测列表中的 anomaly kind。子故障模式显示名可在本机编辑并保存在浏览器本地；恢复措施若为提示词类动作，可点击弹出对应模板全文（含占位符）。设计中尚未实现的检测域不会出现在此表。
