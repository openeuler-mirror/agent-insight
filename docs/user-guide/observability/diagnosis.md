...
- **一键诊断**：同一个 Agent 先完成五模块诊断并冻结主诊断结论，再由它调用 Skill 内适用的专项诊断器、补充语义说明并直接生成最终完整报告。整个过程只启动一次 Agent，系统不会在后台重复运行诊断器或再开一次合并对话。
- **普通追问**：沿用当前对话问答，只结合已有报告、Trace 与历史上下文回答，不重新运行五模块。
- **定向查因**：当问题明确包含循环、反复调用、无进展等已注册症状时，当前追问 Agent 会调用 Skill 内匹配的专项诊断器，并在本次回答中解释结果；不运行五模块。

重复结果会由同一个 Agent 并入原有诊断卡片，其中的区间、次数、比例和证据节点仍完整保留；不重复的问题也使用同一种诊断卡片展示。页面不显示诊断器来源标签。若问题未命中任何专项诊断器，会自然回到普通追问。
