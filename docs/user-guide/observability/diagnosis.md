...
智能诊断由七个部分组成，分别承担"建立全貌、索引证据、输出结论、分层归因、核验 Skill 覆盖、关联推理底座、沉淀事实"的职责：
...
启动智能诊断时，AgentDebug 主诊断与 Skills 步骤核验会并行运行。点击启动后，请求只负责创建后台任务并返回运行态，最终结果由页面轮询对应缓存获得。主诊断结果、Skills 核验结果与推理 Infra 观测互不等待：任一分析完成后，页面会先显示对应区块；另一个区块继续保持运行态并在完成后独立刷新。分析过程中切换页面或刷新浏览器后，再次进入同一 Trace 会从缓存恢复 `running` / `done` / `failed` 状态；若本地服务进程在分析中途重启，任务不会继续执行，页面会提示失败并允许重新分析。
...
推理 Infra 观测卡片会按当前 Execution tree 的推理源 endpoint 与时间窗关联已采集的 infra 指标。它展示 `INFRA-BOUND`、`APP-BOUND`、`INHERENT` 或 `unknown` 等归因标签，并给出队列 p95、KV cache 峰值、抢占率、生成吞吐等关键读数。

这些指标也会进入诊断 Agent 的追问上下文：当卡片显示 `INFRA-BOUND` 时，诊断会把底层排队、KV 压力、抢占或吞吐异常作为根因候选；当显示 `APP-BOUND` 或 `INHERENT` 时，诊断会避免把慢请求误判为 infra 问题。
...
6. 查看推理 Infra 观测卡片，判断慢请求是否与底层推理服务排队、KV 压力或抢占相关
7. 对照模块级诊断卡片，确认问题集中在哪一层
8. 必要时回到原始 Span 详情复核
9. 决定后续动作：转为评测样本、Skills 优化需求，或进一步故障分析
