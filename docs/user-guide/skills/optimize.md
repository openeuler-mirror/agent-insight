...
勾选优化点后可点击「手动优化」执行；在对话框输入补充诉求并点击「发送」（或按 Enter）也会立即启动一次优化，并同时带上当前已勾选的优化点。没有勾选优化点时，也可以只根据补充诉求发起优化。会话或基线文件尚未就绪时，这些入口会暂时禁用；初始化失败会在输入框上方显示原因。
