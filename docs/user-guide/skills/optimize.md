...
在新工作台中，点击左侧 **Skill 优化** 会复用现有优化 Agent，但结果先保存为独立候选快照，不会调用旧“应用草稿”接口。候选会记录静态评估问题、用户要求和关联实验；无高风险静态问题后进入 **待复测**。
...
确认 Diff 和静态质量没有问题后，先点击 **按原实验复测**。平台复制来源实验的数据集、Case 顺序、评估器与运行参数，唯一主动替换的变量是候选 Skill。复测前候选的任务成功率保持“待复测”，不会根据静态规则估算。

只有复测达到来源实验基线后，候选才进入 **复测通过**，并展示 **发布新版本**。再次确认版本号后，例如从 `v1` 发布为 `v2`；复测失败、放弃或版本冲突都不会改变当前激活版本。
