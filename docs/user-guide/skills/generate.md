...
完整链路通常为：**确认生成范围 → 生成并校验 Skill 包 → 查看工作快照 → 保存并发布 → 实验 → 优化候选 → 复测 → 再发布**。
...
- **保存并发布**：生成完成后可直接把当前工作快照追加为正式版本；静态评估、触发分析、用例分析和 A/B 测试均不是生成发布的前置条件
...
生成 Agent 会在完成前执行 `validate_skill.sh`。该校验覆盖 `SKILL.md`、YAML frontmatter、name、description、文档行数、Shell 语法和参考文件说明；确定性错误需要先修复，只有警告时仍可完成。该结果属于生成 Agent 的流程校验，不会被发布接口保存为独立审批状态或再次执行。
...
点击 **保存并发布** 不再弹出第二次确认。页面只在生成仍在执行、当前没有生成会话或尚无文件时禁用发布；后端继续检查会话、文件、`SKILL.md` 和最新正式版本内容是否重复。发布成功后新版本立即成为激活版本，流程解析在后台执行，解析失败不回滚已发布版本。
