...
系统按同一个 Case 和轮次成对调度 A、B 两侧，避免两侧因为排队顺序不同形成不可比的执行窗口；多组配对可以并行。两侧 Trace 生成完成后，所有 `Case × A/B 侧 × 评估器` 统一进入标准批量评估。运行中保留单条执行与评估状态，但顶部 A/B 均分、胜负和最终结论只在全部预期结果收敛后发布；单侧重评会保留上一版稳定分，直到新结果完成后原子替换。
...
- 页面生成单侧 **执行记录**
- 评估器自动重试期间保留该侧上一版已完成分数，只更新失败或缺失的评估器；Case 不会因成功分被清空而临时退回未配对
- 只有 A、B 两侧运行和冻结的全部评估工作都进入终态后，实验才显示 **完成**
