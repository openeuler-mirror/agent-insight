...
从数据集运行时，系统会并发生成所选用例的 Trace；全部可用 Trace 登记完成后，再一次性创建完整的 `case × 评估器` 结果行并进入标准批量评估。运行中可以查看单条明细和固定总量进度，但顶部任务结果分、轨迹分与最终结论会在整批收敛后统一确认，不把阶段性均分展示成最终分。失败评估器的重试只更新失败项，已经完成的分数不会在重试期间消失。

> 未绑定 **评测任务** 时，② 区域保持空状态，不会自动展示历史 Trace 的旧评测记录；创建或选择任务并启动评测后，才会按当前任务显示执行明细。
...
- Trace 列表显示勾选数量
- 启动后，所有已勾选 Trace 会立即进入当前评测任务，不会等上一条完成后才逐条出现
- 从数据集生成的 Trace 也会追加到当前已选择的评测任务，不会另建任务；执行状态与评估分数使用同一个任务口径
- **② 评测执行** 显示 **评测中**、**部分评测**、**评测失败** 等状态
- 多条 Trace 与多个评估器由系统按并发上限在后台执行；运行期间可离开页面，返回后状态会继续刷新
- 进度总数按启动时冻结的 `Case × 评估器` 工作量计算；只有全部结果进入终态后，任务才显示 **已完成**
- 单项自动重试只重跑失败或缺失的评估器，已完成的结果分保持显示，不会在轨迹评估重试时被临时清空
- **③ 分析结果** 展示 **结果分析 均分**、**轨迹分析 均分**、**通过率** 与分组明细
- 行内 **执行 Trace** 与 **评估 Trace** 支持跳转查看完整链路

> 看到 **待评测** 或 **评测中** 时无需重复点击 **开始评测**。模型响应较慢或发生重试时，单项评测可能持续数分钟；页面会持续刷新最新进度，并在全部评估器进入终态后统一完成。
