...
在新 Skill 工作台中，这些能力分成两个清晰边界：**Skill 评估**页签只做静态质量检查；触发分析、用例分析和 Skill A/B 都进入 **Skill 实验**页签的统一实验流程。静态评估不展示任务成功率、触发准确率或运行成本，真实效果只来自实验。旧评测总览在兼容期继续可用。
...
- **分数**：当前有可见卡片分数的维度加权结果；A/B 测试 40%、用例分析 30%、触发分析 20%、静态合规 10%，未完成维度不进入分母
- **覆盖度**：四个维度中已有多少维度完成评测
