...
在 Skill 工作台的统一实验流程中，触发分析只使用预置评估器 **`skill-trigger-analyzer`**。每条用例把 `should_trigger` 标注与真实路由是否命中当前 Skill 直接比较，并且只生成一个名为 **触发准确率** 的评分点：一致得 100 分，不一致得 0 分。正确项显示固定证据；错误项将实际触发观察与数据集原因合并为证据，并给出 **Skill 建议** 和一句话结论。误选、漏选、TPR 与 FPR 只是从同一批路由事实派生的汇总指标，不是额外评估器或评分点。AI 新建的数据集名称包含日期和 `HH:mm:ss`，便于区分同一天连续生成的数据集。

触发分析使用独立的执行上限：最多并发运行 5 条 query，单条等待 30 秒；只有超时时才最多重试 1 次。该口径不继承用例分析和 A/B 测试的通用 Agent 长超时，避免少量异常请求长时间占住整批评测。

运行时，系统会把目标 Skill 安装到本次评测的独立工作目录，并让 OpenCode Session 从创建开始始终绑定该目录。这样模型只会基于本次冻结的 Skill 版本判断是否触发，不会误用用户主目录中的其他 Skill 或文件。
