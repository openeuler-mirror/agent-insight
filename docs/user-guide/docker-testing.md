...
本页面向维护者验证源码打包，不是 830 服务端对外交付安装步骤。830 仅交付经确认的固定 Docker 镜像；用户请按 [Docker 部署服务端](./quickstart#docker-部署服务端830-转测交付) 安装。
...
> [5 分钟上手 · 维护者源码模式](./quickstart#6-维护者可选docker-源码挂载模式)。
...
完成维护者测试后，对外交付应使用交付清单中已验证的固定版本镜像，并附目标架构与离线包 SHA256；不要把临时测试镜像或浮动 tag 当作 830 交付版本。用户安装流程见 [Docker 部署服务端](./quickstart#docker-部署服务端830-转测交付)。
