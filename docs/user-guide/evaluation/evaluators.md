...
- **Agent 轨迹质量**：面向 **Skill 与旧轨迹评测** 场景，保留既有完整性、工具选择和冗余度三维 opencode 评分口径。
- **Agent 执行过程质量**：面向通用 **轨迹实验** 场景，使用 **目标对齐、规划完整性、推理连贯性、异常处理、路径稳健性、信息利用** 六个维度做 0～100 分评估。它使用独立 ID，只出现在新实验入口，不改变旧轨迹质量评估器。
- **Agent 步骤效率**：面向 **轨迹** 场景，使用 **步骤必要性、路径绕行、成本效率、步骤密度、重试效率** 五个维度做 0～100 分评估。它只出现在新实验入口，不进入旧轨迹批量评测入口。
...
Agent 步骤效率与 Agent 执行过程质量两张新卡都按 0～100 展示，但内部不是让 Judge 自由打连续分，而是先判每个维度的 `met / partial / missing`，再由代码映射为 100 / 50 / 0 并应用封顶规则。平台展示口径是：总分 **80 分及以上** 为“达成”，**60～79.9 分** 为“部分达成”，**低于 60 分** 为“未达成”。

`preset-agent-process-quality` 使用 `agent-process-quality/1.0.0` 六维 rubric，`preset-agent-step-efficiency` 使用 `agent-step-efficiency/1.0.0` 五维 rubric。原 `preset-agent-trace-quality` 的名称、评分实现和历史结果均保持不变；两个质量评估器使用不同 ID，不互相重算或覆盖结果。

评估失败与低分不是一回事。轨迹过长超过上下文限制、Judge 返回的 JSON 不符合契约、或模型连接未配置时，系统会把该次评估标记为失败，而不是把 Agent 记成 0 分。只有评估真正成功后，卡片才会展示 0～100 的结果分和评分点证据。

如果你需要追溯这两张卡的需求来源与实现边界，可继续阅读 [Phase 1 需求分析](../../design/agent-trajectory-evaluation/phase1-requirements-analysis.md)、[Phase 2 需求设计](../../design/agent-trajectory-evaluation/phase2-requirements-design.md)、[Phase 3 开发计划](../../design/agent-trajectory-evaluation/phase3-development-plan.md)。
...
- 通用 Agent 实验优先使用 **Agent 执行过程质量** 评估目标对齐、规划和异常处理是否可靠；Skill 或旧轨迹入口继续使用 **Agent 轨迹质量**。
- 需要比较“是否绕路、是否原地重试、是否有可避免成本”时，再补充 **Agent 步骤效率**。
