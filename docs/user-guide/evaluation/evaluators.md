...
- **文本简洁性**：按表达效率、套话精简、主需求聚焦和信息完整四个维度评分；过度精简造成关键信息缺失时会限制总分。

四个文本质量评估器均由 Judge 以温度 0 输出离散等级，再由代码确定性计算 0～100 分，以降低同一文本重复评估时的采样漂移。卡片顶部展示不超过 80 字的具体总结；每个评分点展开后显示中文理由、原文引用和改进建议，不额外输出卡级证据。AI 味、格式和语种一致性沿用安全评估器的扣分聚合：最大扣分完整计入，其余扣分按维度数均摊追加。格式评估器将序号连续性和引用正确性设为关键维度；语种一致性将主语言匹配设为关键维度，其余为普通维度。缺少语种评估所需的用户问题时直接失败。简洁性按 0.3/0.2/0.3/0.2 加权，并对严重冗余、偏题和信息缺失设置总分上限。
