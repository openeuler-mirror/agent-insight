...
- **Agent RAS 可靠性故障注入评估器**：读取已有或新生成的 Trace 与故障事实，只评价“故障发生”；该维度得分就是评估器总分。
- **Agent RAS 可靠性故障检测恢复评估器**：评价“故障检测、触发处置、故障消解”，评估器总分是三个维度的算术平均。
...
评测结果统一以评估器卡片展示：卡片顶部先给出一句话总体摘要和总分，展开后按评分点表格呈现维度、得分和证据。回答深度性、轨迹工具利用率、工具选择合理性和 Agent RAS 可靠性评估器在有评分点时不直接展开原始结构化 JSON；完整证据仍保存在评估结果中，用于诊断和后续扩展。两个 RAS 评估器同时执行时，与其他通用评估器一样，实验综合分按两个评估器总分求平均；旧“Agent RAS 可靠性评估器”仅用于历史结果兼容，不再出现在新建实验的可选列表中。
