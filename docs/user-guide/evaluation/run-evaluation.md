...
在实验创建向导中选择 Agent 并开启「监听模式」，可以不圈选历史 Trace，直接创建实验。服务端约每 5 秒分批扫描数据库，不需要保持浏览器页面打开。

自动加入实验的 Trace 必须属于当前用户、匹配所选主 Agent，且链路跟踪状态为「已完成」。其执行开始时间须晚于监听开启时间；未上报执行开始时间时，使用首次入库时间，后续补传不会刷新该入库时间。因此，未提供开始时间的历史 Trace 若在监听开启后才首次上传，也可能被纳入。

执行中、超时和失败的 Trace 不会自动加入。延迟上传或稍后才更新为已完成的 Trace，会在后续扫描中被发现。同一 Trace 在同一实验只自动加入一次，但可以分别进入多个匹配实验。

自动监听只是代替用户选择已有 Trace，后续数据读取、评估器调用、正常重试及结果保存与手动评测一致；不会重新运行 Agent 或修改原始 Trace。评估器失败按现有规则展示失败结果和提供重评操作，监听扫描不会额外重评已有 Case，也不会覆盖人工评分。

监听中的实验详情页会自动刷新。点击「停止监听」后不再自动加入新的 Trace，已经开始的评测继续完成，已有结果保留。该功能不依赖 OpenCode 等特定框架的上传回调；只要 Trace 已正确入库并满足条件，即可参与评测。
