...
生成 Trace 的运行主机来自“客户端配置”，页面不会再提供 Worker 安装 curl 命令。如果没有可用 IP，请先检查客户端是否在线，以及其能力上报是否包含所选 Agent、平台和模型。普通生成还要求客户端声明“可返回 Trace ID”；旧客户端即使能执行 Agent，也不会作为普通生成目标，需要升级客户端后再试。
