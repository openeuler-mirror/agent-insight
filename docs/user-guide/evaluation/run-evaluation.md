...
实验详情页只负责展示运行状态、进度和结果，不再提供第二个“开始执行”按钮，也不在内容区重复提供“返回实验列表”按钮。实验没有可操作的“草稿”阶段：创建与启动之间的临时记录不会出现在实验列表；启动失败时系统会自动回滚仍未启动的临时记录并保留向导内容，用户再次点击时会重新创建并启动。只有服务端确认已进入运行流程后才跳转详情页；如果启动实际已生效但响应丢失，系统会保留该实验并进入详情，不会误删运行中的数据。
