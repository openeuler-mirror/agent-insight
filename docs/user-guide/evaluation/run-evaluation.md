...
1. **实验设计**：填写实验名称、选择待执行 Agent，并按需选择内置可靠性数据集。Agent 下拉同时包含已有历史 Trace 的 Agent，以及客户端配置中在线主机扫描到的可执行 Agent。
2. **Trace 来源**：
   - **选择 Trace**：保留自动监听和历史 Trace 筛选、分页、多选能力；开始实验后直接评估，不重新运行 Agent。
   - **生成 Trace**：先选择运行主机 IP，再选择该主机上当前 Agent 可用的模型，并勾选数据集 Case。未选择数据集或没有在线可执行主机时不可使用。
3. **预期答案**：可靠性数据集会自动带入所选 Case 的预期答案；选择已有 Trace 时按任务输入精确匹配。未选择可靠性数据集时，可继续使用“从数据集导入匹配”、手工标注和“存为数据集”。
4. **评估器与执行**：可靠性数据集提供两个独立可选项——“Agent RAS 可靠性故障注入评估器”只判断故障是否发生，“Agent RAS 可靠性故障检测恢复评估器”判断故障检测、触发处置和故障消解；可单选或同时选择，系统只执行实际勾选项。选择已有 Trace 时也可使用这两个评估器。点击“开始实验”后系统立即启动：已有 Trace 直接进入评估，生成 Trace 则立即进入运行中并在后台完成 Trace 生成、绑定和评估。

实验详情页只负责展示运行状态、进度和结果，不再提供第二个“开始执行”按钮，也不在内容区重复提供“返回实验列表”按钮。启动请求失败时，创建页会保留当前实验并允许重试，不会重复创建实验；只有服务端确认已进入运行流程后才跳转详情页。

生成 Trace 是可靠性评估的硬前置条件。系统只有在 Case 已关联到 Execution，且对应 Session 含非空轨迹时才启动该 Case 的评估。全部 Case 都未生成有效 Trace 时，实验标记为失败且不产生评估分；部分 Case 成功时，只评估成功 Case，失败 Case 在详情中显示“Trace 生成失败”，不计入综合分。详情状态条会分别展示 Trace 的已生成、失败和生成中数量。

实验列表的“综合分”展示当前实验全部有效结果的综合得分。它与通用评估器使用同一口径：先由每个 RAS 评估器计算自己的总分，再对实际执行且有分的评估器总分求平均；Trace 评测详情中的每张 RAS 评估器卡会明确显示该评估器总分，展开后可查看各维度证据和建议。

生成 Trace 的运行主机来自“客户端配置”，页面不会再提供 Worker 安装 curl 命令。如果没有可用 IP，请先检查客户端是否在线，以及其 inventory 是否包含所选 Agent、平台和模型。
