...
> 适用范围：`agent-core` Agent RAS（`LlmThinkingLoopDetector` + 自动恢复 + abort）  
> 对比基准能力来源：原 Thinking Loop Lock 生产 case / 实测数据；落地形态以当前 harness 为准。
...
Agent RAS 通用骨架是四层：**Rail（收集）→ Monitor（编排）→ Detection（检测）→ Recovery（恢复）**。  
LLM 思考死循环只是 Detection 里的一个 **Detector 实例**；工具循环等共用同一 Rail/Monitor/Recovery，仅 Signal 来源与 Detector 不同。
...
| 层 | 模块 | 职责 |
|----|------|------|
| **Rail** | `rails/agent_ras_rail.py`、`signal_builder.py`、`stream_observer.py` | 采集生命周期与流事件 → Signal；创建/销毁 Monitor；**不做**检测与恢复决策 |
| **Monitor** | `monitor.py` | 编排 `detection` → `recovery`；L1/L2 直接 abort；L3 启动 recovery skill；思考循环走 `on_stream_chunk` |
| **Detection** | `detectors/*`（本方案实例：`llm_thinking_loop.py`） | Signal → `Anomaly`；L1/L2 同步、L3 异步首次 Judge |
| **Recovery** | `recovery/engine.py`、`operations.py`、`robustness_prompt.py`、`recovery/skills/llm-loop-review` | 映射并执行 `SUPPRESS_STREAM` / steering；L3 recovery 二次复核（`llm-loop-review`） |
...
| 层 | 路径 |
|----|------|
| Rail | `harness/rails/agent_ras_rail.py`、`harness/agent_ras/signal_builder.py`、`harness/agent_ras/stream_observer.py` |
| Monitor | `harness/agent_ras/monitor.py` |
| Detection（本方案实例） | `harness/agent_ras/detectors/llm_thinking_loop.py` |
| Skill 注册表 | `harness/agent_ras/agents/base.py`（故障域 × `detection`/`recovery` → skill 名） |
| Recovery | `harness/agent_ras/recovery/engine.py`、`operations.py`、`robustness_prompt.py` |
| 配置 | `harness/agent_ras/config.py` → `LlmThinkingLoopConfig` |
| Core 停流契约 | `core/.../rail/base.py` abort API；`react_agent.py` 流循环消费 |
