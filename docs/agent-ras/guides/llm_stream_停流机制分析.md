...
- [`agent-core-rail-base修改说明.md`](./agent-core-rail-base修改说明.md) — core 侧最小改动（`request_abort_stream`）
- [`llm_thinking_loop_方案说明.md`](./llm_thinking_loop_方案说明.md) — LLM 思考死循环场景细节
...
- `openjiuwen/harness/agent_ras/monitor.py`
...
- `close_stream` 后 harness **收不到**后续 chunk，thinking-loop 检测也停止
...
| 层级 | 内容 | 归属 |
|------|------|------|
| 语言标准 | `stream_iter.aclose()` | **Python 异步生成器标准方法**（`AsyncGenerator.aclose`），非本仓库、也非本分支造的 API |
| 仓库既有 | `llm.stream(...)` 为 async generator；客户端 `stream()` 的 `finally` 里 `async_client.close()` | **baseline（如 `enterprise-dev`）已有**；本分支未改该清理路径本身 |
| 本分支新增 | `request_abort_stream` / `consume_abort_stream` / `has_abort_stream_request`；ReAct 在 abort 后 `break` 并**主动调用** `aclose` | **相对 baseline 的 Agent RAS 停流契约** |
...
harness/agent_ras/monitor.py
  ...
  → react_agent.py chunk 边界 break + aclose
```
...
| 2026-07-23 | 补充归属区分：`aclose`=语言标准；客户端 close=`enterprise-dev` 既有；abort 信号与主动 `aclose`=本分支 |
