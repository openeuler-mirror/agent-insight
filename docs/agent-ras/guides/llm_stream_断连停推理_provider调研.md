...
- [`llm_stream_停流机制分析.md`](./llm_stream_停流机制分析.md) — 本仓库 abort / `aclose` 停流契约
- [`llm_thinking_loop_方案说明.md`](./llm_thinking_loop_方案说明.md) — LLM 思考死循环检测与恢复
- [`agent-core-rail-base修改说明.md`](./agent-core-rail-base修改说明.md) — `request_abort_stream` core 改动
...
1. 检测异常并 `request_abort_stream`（**本分支**新增信号，见 `rail/base.py`）
2. ReAct 在 chunk 边界 `break`（**本分支**在流循环中消费 abort）
3. 调用 `stream_iter.aclose()`（**语言标准**方法；触发的客户端 `close()` 为**仓库既有**清理路径）

**归属区分：** `aclose` ≠ 本分支/本仓库自造 API；本分支新增的是 abort 信号与「abort 后主动调用 aclose」。详见 [`llm_stream_停流机制分析.md`](./llm_stream_停流机制分析.md) §6.2。
...
| 2026-07-23 | 落盘至 `docs/agent_ras/`；补充 `aclose` / abort 信号的本分支 vs 仓库既有 vs 语言标准归属 |
