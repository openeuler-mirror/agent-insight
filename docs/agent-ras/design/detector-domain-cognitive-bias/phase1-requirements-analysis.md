...
> 关联文档：
> - [规划错误 / Planning Error](../detector-planning-error/phase1-requirements-analysis.md)
> - [语义层故障注入调研](../detector-analysis-paralysis/phase1-fault-injection-survey.md)
...
| 规划错误（Planning Error） | 策略层不可行 / 忽略约束 | ⚠️ 方案中（[PE phase1](../detector-planning-error/phase1-requirements-analysis.md)） |
...
- 为故障注入评测页提供可执行剧本（对齐 [MAS-FIRE / AutoInject 调研](../detector-analysis-paralysis/phase1-fault-injection-survey.md)）。
...
> 开放场景很少有「正确答案」作 oracle。评测真值来自**注入剧本标签**（`fault_type` + `injection_point` + `expected_detection`），而不是领域金标准答案。通用语义注入机制深拆见 [phase1-fault-injection-survey.md](../detector-analysis-paralysis/phase1-fault-injection-survey.md)。
...
13. **Jia et al.** "MAS-FIRE: Fault Injection and Reliability Evaluation for LLM-Based Multi-Agent Systems." arXiv:2602.19843, 2026. 数据：https://github.com/wxhhxn/MASFIRE；本仓拆解见 [phase1-fault-injection-survey.md](../detector-analysis-paralysis/phase1-fault-injection-survey.md)  
...
18. [Planning Error phase1](../detector-planning-error/phase1-requirements-analysis.md)（含同构的故障注入章节）  
19. [AgentDebug 错误词表](../../../../skills/agent-debug-diagnosis/references/02-error-taxonomy.md)  
20. [语义层故障注入调研](../detector-analysis-paralysis/phase1-fault-injection-survey.md)
---
