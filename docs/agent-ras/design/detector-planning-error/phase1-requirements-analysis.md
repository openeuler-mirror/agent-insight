...
> 关联调研：[语义层故障注入调研](../detector-analysis-paralysis/phase1-fault-injection-survey.md)
...
> 开放域很少有「金标准计划」。评测用真值来自**注入剧本标签**（注入点 + 期望 `primary_fault`），而不是世界最优路径。通用语义注入机制的深度拆解见 [phase1-fault-injection-survey.md](../detector-analysis-paralysis/phase1-fault-injection-survey.md)。
...
11. 本仓 [语义层故障注入调研](../detector-analysis-paralysis/phase1-fault-injection-survey.md) — AutoInject / MAS-FIRE 机制深拆与过度思考注入草案。
