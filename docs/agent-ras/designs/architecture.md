...
**在范围内**：环内异常检测与恢复；多平台 HostControl / AgentAdapter；同进程 `ras_runtime`；① `ras-events` 旁路与能力配置拉取。

**不在范围内**：用 OTLP **替代**环内恢复；RAS/FI 承担日常完整 ⓪ Trace（xiaoO 走 Insight collector）；为每个平台重写检测算法；fork openjiuwen.core；Insight 看板实现（见 [`docs/design/`](../../design/) 与 developer-guide）；运行时人工 HITL 确认（现行为自动 L3 Reviewer / wire 投递）；独立 RAS HTTP 守护进程。
...
| 关系 | 基数 |
|------|------|
| runtime : 宿主进程 | 通常 1 : 1（OpenCode / openjiuwen） |
| hooker 子进程 : SessionHub | xiaoO：N : 1（`subprocess_ipc` worker 共享 Hub，见 §4.7） |
| session : runtime | N : 1（`{platform}:{native_id}`） |
| Agent Insight : runtime | 旁路 ① ras-events，fail-open |

runtime 随宿主进程初始化与释放；不监听 RAS 端口，不写 sidecar PID/锁文件。xiaoO CLI hooker 是短生命周期子进程，**不能**每进程各建 Hub，否则 repeat-tool 连续计数会断档。
...
## 4. 协议 inproc：嵌入运输与模块调用

openjiuwen **深挂载不走本节**（进程内已是 Python，直连 `core/monitor`）。协议 inproc 有三种嵌入运输，勿混：

| 运输 | 谁用 | 本节 |
|------|------|------|
| libpython FFI | OpenCode（Bun 同进程） | §4.1–4.5 |
| 本机 Python `import ras_runtime` | xiaoO Daemon 等长生命周期 Python 宿主 | §4.6 |
| `subprocess_ipc` | xiaoO CLI hooker 短生命周期子进程共享 Hub | §4.7 |
...
| 解释器 / 运输 | libpython（JS）/ 本机 Python / hooker `subprocess_ipc` | 宿主已是 Python |
...
[`platform_adapter/common/ras_client.py`](../../../agent_ras/platform_adapter/common/ras_client.py) 同进程 `import ras_runtime`，JSON 编解码与 JS client 对齐；xiaoO Daemon 路径用它 + `CallableHostControl`。不加载 `python_bridge.js`，也**不**再 dlopen libpython（解释器已是宿主）。L3 Judge 与 OpenCode 同协议：observe 可带 `skill_requests`，宿主 `RasClient.skill_result` 回灌 Hub。

验证：`bash agent_ras/scripts/smoke_inproc.sh`（显式 `unset LD_PRELOAD`，证明 RTLD_GLOBAL 路径成立）。

### 4.7 子进程共享 SessionHub（`subprocess_ipc`）

这是 **xiaoO CLI hooker 的稳定嵌入运输**，不是「实验性 out-of-process RAS 守护进程」，也不是 §4.1–4.5 的 libpython FFI。

xiaoO gateway 会为 hook 场景 fork 短生命周期 Python 子进程。若每个子进程各自 `ensure_runtime()`，SessionHub 状态（tool 连续失败计数、流式累积窗口）会按进程撕裂。因此：

1. hooker 子进程调用 `ras_runtime.call` 时，若本机 IPC worker 可用（`ipc_available()`），`facade.call` **转发**到 Unix socket worker，而不是在本进程建 Hub。
...
## 5. 与 Insight / FI 的通道（架构不变量）

RAS 只做环内决策 + 旁路事件，**不**造 Insight Execution 主树。通道切分：

| 通道 | 谁负责 | RAS 做什么 |
|------|--------|------------|
| ⓪ Trace（完整 Execution/Session） | Insight 采集器（OpenCode upload；xiaoO `xiaoo-trace-collector`） | **不做**日常 OTLP / `note_*` |
| ① ras-events | `insight_push` / InsightAnomalyReporter | fail-open；ingest 失败不挡 abort/notice |
| 能力配置 | Insight `GET /api/ingest/ras-config?platform=` | 客户端合并进 hello；显式 `semantic_content_enabled=false` 才关 L3 |
| ③ FI collect-result | FI Worker + Insight Judge | RAS **不**被 FI Worker 拉起 |

Join：RAS `taskId` 与宿主 native session id 对齐（xiaoO 去 `xiaoo:` 前缀）。评测 overlay 与通道细节见接入方案，本文不重复。
...
完整加载与文件调用见 **§4**。此处仅保留编排摘要（`skill_result` 回灌见 §4.5 正文，不在本图展开）：
...
- 单 Agent：`create_deep_agent(agent_ras=AgentRASConfig(...))` / `build_agent_ras_rail`
- OpenCode：`install-ras` + libpython inproc + ras-judge
- xiaoO：`install-ras` hooker + Daemon SSE 控制面；hooker 经 `subprocess_ipc` 共享 Hub
- 检测：`repeat_tool`、`llm_thinking_loop`（L1/L2 字面 + 可选 L3 检测 skill）；`semantic_content_enabled` 默认 true
- L3 Judge：协议 inproc 为 `HostCallback` + `skill_result`（OpenCode ras-judge / xiaoO `RasClient.skill_result`）；深挂载为 `DeepAgentAdapter`
- 流检测（深挂载）：`StreamObserver` 在 `write_stream` 前转发 `Monitor.on_stream_chunk`；xiaoO 正式 mid-stream 为 Daemon SSE
...
**组件边界**：Rail/钩子/Daemon SSE 采 Signal；Monitor **或** SessionHub 编排；`recovery` 决策；Host 投递。稳定公共 API：`AgentRASConfig`、核心模型、`RecoveryAction`、`HostControl`、`build_agent_ras_rail` / `ras_runtime.call`。

**数据边界**：检测链路不递归脱敏；evidence 最小原则；旁路 push fail-open；RAS 不发日常 OTLP。

**实验 / 不做**：独立 RAS HTTP 端口或 sidecar 守护进程不在稳定承诺内。`subprocess_ipc` **不属于**这类实验项。
