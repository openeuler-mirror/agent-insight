...
**Agent-insight** 正是为了解决这些问题而生的。它是 openEuler 社区推出的开源项目，定位为面向 Skill 全生命周期管理的工具平台，覆盖 Skill 的"生成 → 评测 → 优化"闭环，让执行过程数据成为 Skill 持续进化的核心驱动力。
...
Agent-insight 提供三大核心能力，形成 **生成 → 评测 → 优化** 的完整闭环。

### 1. 基于语义聚合的 Skill 生成

针对大量相似文档生成冗余 Skill 的问题，Agent-insight 采用"去冗余、合相似、抽模式"的方法：
...
不同于只看结果的评测方式，Agent-insight 构建了涵盖准确率、时延、Token 成本、投入产出比等多维度的评测体系，并提供过程级可追溯能力：
...
当执行过程被结构化记录后，优化就不再依赖猜测。Agent-insight 能够：
...
Agent-insight 整体由以下几部分组成：

- **Meta Skill**：Skill生成、优化的元能力。
- **数据采集层**：通过插件或日志旁路，从各 Agent 框架中采集执行过程数据。
- **Agent-insight 平台**：提供界面化的多维观测、配置管理和深度分析，分析结果可用于持续优化。
