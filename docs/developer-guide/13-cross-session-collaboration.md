...
通用 Trace 详情读取 `/api/observe/session` 时，会调用 `findGoalPlusTraceProjectionMembers` 查询当前主 Execution 作为 `from` 端且两端都已 linked 的 `goal-plus-semantic/orchestrated` 事件。随后 `composeCollaborationTrace` 在响应内追加一个带 `trace_synthetic`、`trace_relation` 元数据的虚拟 TASK，以及 worker 原生交互的展示副本，现有 Agent Tree 因此可以渲染 `TASK → AGENT → LLM/TOOL` 层级。`full`、`structure`、`interactions` 和按索引读取单条 interaction 使用同一套确定性投影，避免懒加载时结构索引与正文错位。
