...
`POST /api/ingest/collaborations/events` 只接受有效 `x-witty-api-key`，请求最多 64 KiB。外部请求不能提供 sourceType，因此永远保存为 `reported`。相同 collaboration/event ID 与相同规范化正文返回 200 duplicate；正文不同返回 409 `EVENT_CONFLICT`。
