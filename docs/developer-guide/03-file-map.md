...
|---|---|---|
| `benchmark/{adapter-base,adapter-registry,dataset-service,experiment-service,orchestrator,scheduler,run-callback-service,evaluation-preparation-service,evaluator-target,evaluator-runtime-config,evaluation-scheduler,evaluation-callback-service,experiment-result-service}.ts` | benchmark | 五方法 `AbstractBenchmarkAdapter`、Catalog 驱动的 `getBenchmarkAdapter`、数据集/实验/Run 编排、执行与评测回调、热加载目标快照、Raw Result 持久化、统一结果投影、固定分母聚合和证据读取 |
