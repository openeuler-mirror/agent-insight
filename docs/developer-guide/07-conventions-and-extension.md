...
- **新增 Benchmark**：只新增 `benchmarks/<key>/` 接入包，至少包含唯一真源 `benchmark.yaml`、实现五个业务 hook 的 `adapter/index.ts`、`schemas/{case,result}.schema.json`、`evaluator/evaluator.yaml` 和遵循 `doctor` / `evaluate --request ... --output ...` 文件契约的 Entrypoint。Adapter 必须生成完整 Agent Prompt；执行器只按 workspace provider、Agent platform 和 Artifact collector 选择通用能力。新判分逻辑放在自己的 `evaluator/*`；能复用已有 Evaluator 时只声明引用。增加真实 Case smoke 和 conformance 后运行 `npm run benchmark:catalog`、相关 API 测试及 `npm run test`。禁止手改 `generated/benchmark-catalog/*`，也不修改 Benchmark 公共 API、调度器、执行器 Runner、评测 Worker、Prisma Schema 或其他 Benchmark。只有现有工作区、Agent Runtime 或 Artifact Collector 无法表达需求时，才新增一个按能力命名、可跨 Benchmark 复用的执行器组件。完整契约见 [`benchmark-extension-refactor-plan.md`](../../评测服务文档/benchmark-extension-refactor-plan.md)。
- **Benchmark 数据集与展示**：有官方数据集时，在 `benchmark.yaml` 声明 `dataset.profiles` 和 `presentation`；特殊格式再提供 `dataset/index.ts` 的 `BenchmarkDatasetLoader`，JSON/JSONL 可使用平台通用 Loader。Adapter 的 `validateAndSplitCase()` 同时返回公开 `catalogProjection`。管理员用 `scripts/benchmark/install-dataset.ts` 从任意可读文件导入一份系统共享数据；用 `remove-dataset.ts` 删除未引用数据或归档已引用数据。Case 表格、搜索、参考契约和主指标均由 Presentation 驱动，禁止接入包注入 React、HTML 或 JavaScript。
