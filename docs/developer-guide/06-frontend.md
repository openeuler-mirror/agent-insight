...
- **实验向导** — `app/(main)/experiments/new/page.tsx` 使用“实验设计 → Trace 来源 → 预期答案 → 评估器与执行”四步结构。第 ① 步的 Agent 来自 `/api/experiments/agents`：用户归属历史 Trace Agent 与全部在线客户端 inventory 可执行 Agent 按名称合并，返回历史框架、Trace 数及逐主机 `targets`。数据集可选，当前向导仅展示内置可靠性数据集。第 ② 步在 `existing` / `generate` 间切换：已有 Trace 路径继续通过 `/api/experiments/traces` 服务端分页，搜索同时匹配 `Execution.id`、`taskId` 与 `query`，时间支持预设窗口和自定义起止时间，用户标签多选使用 AND 语义，跨页选择受 500 条上限保护；生成路径从 Agent target 选择 `workerId + platform + host`，模型只读取该 target 上报的列表，Case 使用独立 `selectedGenerated` Map。可靠性已有 Trace 在加入选择集时按输入精确带入数据集预期答案和 FI 元数据；生成 Trace 的预期答案使用数据集快照。第 ④ 步对可靠性数据集只展示故障注入与故障检测恢复两个独立评估器，不自动勾选或绑定；已有 Trace 路径也可选择它们。“开始实验”串行完成 create 与 run，run 成功后才进入详情；失败重试复用已经创建的实验。筛选不持久化为监听规则，监听运行时仍会拒绝系统归属 Agent 的新 Trace。
- **实验结果** — `app/(main)/experiments/page.tsx` 展示 API 返回的 `overallScore`。RAS 与通用评估器统一走 `detail-agg.ts::overallAverage`，按评估成功且有分的评估器生效总分（`humanScore ?? score`）求平均。Trace 评测详情把机器/人工生效分标记为“总分”，并对旧、新 RAS evaluator id 隐藏卡级结构化 evidence 摘要，只保留评分点证据与建议。详情页是纯状态/结果视图：不再为 `draft` 提供“开始执行”，内容区也不重复渲染“返回实验列表”；运行中的实验通过轮询自动刷新进度。
