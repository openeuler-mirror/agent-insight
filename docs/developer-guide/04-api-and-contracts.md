...
- **Agent candidates**: `src/app/api/experiments/agents/route.ts` 将两类来源按 Agent 名合并：当前用户 root Trace 中的用户归属 Agent，以及 `listWorkerExecutionTargets` 从全部在线客户端 inventory 展开的可执行 Agent。历史 Trace 仍通过 `buildExecutionOwnershipWhere('user')` 排除 `RegisteredAgent.agentOwnership='system'` 与内置系统 Agent；`ras-judge` 不进入可执行候选。每项返回 `name`、`traces`、`frameworks`、`executable` 和逐客户端 `targets[]`；target 包含 `workerId`、显示 IP、平台、该主机模型列表与最后心跳时间。OpenCode inventory 优先启动仅绑定 loopback 的短生命周期 headless server，并从 `GET /agent` 读取插件解析后的非隐藏 Agent；失败时回退 `opencode agent list`，同时过滤 `ras-judge/compaction/summary/title`。客户端每 30 秒强制绕过能力缓存，重新同步普通 capabilities 与 FI inventory；配置、插件与 Agent 目录指纹仍用于事件触发的快速刷新。inventory 探测在独立 Node 子进程中运行，避免同步 CLI 阻塞主进程的心跳和长轮询；macOS 后台服务再通过独立的 `launchctl submit` helper 读取与交互式 OpenCode 一致的 resolved catalog。`REFRESH_CAPABILITIES` 使用同一隔离刷新路径。inventory 模型选项以完整 `provider/model` 为 `id`，并在 `label` 后追加 provider，避免不同 provider 的同名模型在实验向导中无法区分。总数上限 50。
...
- **RAS client control plane** (`/api/reliability`): 常驻客户端的注册、控制通道与配置下发。Web：`POST /install-tokens`（一次性令牌，只存哈希）、`GET /clients`、`GET/PUT/DELETE /clients/:clientId/config`、`POST .../config/sync`、`GET /config-schemas/:platform`。客户端（`Authorization: Bearer <deviceCredential>`）：`POST /client/v1/register`、`POST /client/v1/heartbeat`、`PUT /client/v1/capabilities`、`GET /client/v1/commands/next`（长轮询兜底）、`POST /client/v1/commands/:commandId/status`、`GET /client/v1/config-snapshots/:configRef`、`POST /client/v1/config-loads`。WSS 控制通道 `/api/reliability/client/v1/control` **不是** Route Handler —— Next 拿不到 `upgrade` 事件，由 `scripts/control-server.js` 承载（内置 RFC 6455 实现 `scripts/ws-endpoint.js`，无第三方依赖），并通过 `control-auth`/`control-presence`/`control-receipt` 三个内部接口回调 Next。
  **平台登录模式契约**：`GET /api/eval/config/status?check_login=true` 返回 `login_mode`（并保留旧 `check_org`/`org_mode` 兼容）；`src/lib/auth/login-mode.ts` 严格区分 `standalone`、历史 `organization` 与 `idaas_oauth`。历史组织集成仍由 `ORGANIZATION_MODE + ORG_*`、`/api/auth/organization` 和组织 Skill 路由负责；IDaaS OAuth 登录只使用 `LOGIN_MODE=idaas_oauth + IDAAS_OAUTH_*` 与 `/api/auth/idaas-oauth/{authorize,callback,complete}`，并以 `/callback` 作为推荐回调入口、保留原 callback 路径兼容；两者同时开启时失败关闭。IDaaS callback 校验签名 state，以同一 redirect URI 换 token，userinfo 的非空 UUID 去除首尾空白后直接映射 `User.username`；access token 不持久化，callback 用短时签名 HttpOnly Cookie 交给 complete，再返回既有 API Key 上下文。IDaaS 模式下 `POST /api/auth/apikey` 只接受与已有用户匹配的 `x-witty-api-key` 做恢复；请求中的 UUID 只去除首尾空白并保持大小写，与 API Key 对应用户精确匹配，不能按邮箱自注册绕过 OAuth。可选的 `IDAAS_REGION_ACCESS_*` 地区限制在 callback 创建用户前及 API Key 恢复凭据验证后执行：人员请求固定使用 `{ uuids: [uuid] }`；欧盟返回 `region_restricted`，IAM/人员接口异常、空数据或字段缺失返回 `region_check_unavailable`，两者都失败关闭。IAM token 缓存 10 小时，人员记录缓存 2 小时；专用 `undici.Agent` 按 `IDAAS_REGION_ACCESS_TLS_VERIFY` 控制证书校验，不修改全局 TLS 行为。三个 IDaaS 路由对浏览器继续返回通用错误，服务端 `[Auth/IDaaS]` 日志通过 `describeIdaasOAuthError` 输出异常类型与消息，用于定位缺失或无效的变量名；日志不得输出配置值、client secret 或上游请求 URL。开发启动的 `sync_admin_api_key.js` 必须先以 `check_login=true` 确认服务就绪；IDaaS 模式跳过 admin Key 创建，只更新 Host 并保留已有客户端 Key，standalone 与历史 organization 维持原初始化。

  **登录、setup 与 OpenCode 上传凭证契约**：standalone 模式下 `AuthProvider` 恢复 `localStorage.user_id` 时必须调用 `POST /api/auth/apikey` 刷新服务端当前 API Key，`authReady` 前安装页不得生成命令；组织模式以 `/api/auth/organization` 为权威。`GET /api/ingest/setup?key=...` 在脚本生成前调用 `findUserByApiKey`，无效 Key 返回 401，脚本响应带 `Cache-Control: no-store`；显式 `nokey=1` 保留共享账号/兼容流程。`scripts/install-ras-client.js` 每次消费新安装令牌都重新注册，服务端按 `machineId` 复用记录并换发、撤销旧设备凭证。`scripts/opencode_uploader_client.js` 仅在目标服务基址与 `client/config.json.insightBaseUrl` 归一化后完全相同（协议、主机、端口和 basePath 均一致）时发送设备凭证，并统一上报到 `<Host>/api/ingest/upload`（保留 basePath）；缺少签发基址或仅 origin 相同但 basePath 不同都会退化为 API Key 鉴权。`POST /api/ingest/upload` 对 OpenCode 的 API Key 与设备凭证执行双鉴权：任一有效即可接收；失效凭证不建立客户端绑定；两份都有效但账号不同返回 403；两份都失效保持 401/设备鉴权错误。
  **delivery 状态的权威来源分工（不可越权写入）**：`saved`/`sync_notified`/`notify_failed` ← 服务端；`pulling` ← 客户端拉取快照；`written` ← 客户端回执 `SUCCEEDED(state=WRITTEN)`；`ras_loaded` ← RAS `config-loads`。服务端**不得**代客户端标记 `written` —— 页面据此区分「已写入」与「已生效」。指令送达以应用层 `RECEIVED` 为准，ACK 窗口超时记 `DELIVERY_FAILED`，离线不排队、不在重连后补发。白名单 action：`APPLY_CLIENT_CONFIG`/`PREPARE_EXPERIMENT_CASE`/`RUN_EXPERIMENT_CASE`/`REFRESH_CAPABILITIES`；配置类 payload 禁 `url/config/path`，运行类禁 `command/shell/args/cwd/executable`（双端各校验一次）。客户端：`scripts/reliability-client.cjs`（吸收 FI Worker 采集职责；Python 缺失只降级 `faultInjection.ready=false` 而非退出；可靠性 Case 与 FI run 互斥）；安装与保活：`scripts/install-ras-client.js`（systemd `Restart=on-failure`+`WatchdogSec`，launchd `KeepAlive`；仅 Linux/macOS；系统 Python 只创建版本化 managed venv，FI pip/CLI/xiaoO Hook 固化到 `~/.agent-insight/fault-injection/runtimes/<id>/venv`，不回退全局 Python；launchd 对 bootout/bootstrap 竞态重试并以 `launchctl print` 验证；systemd/launchd 都固化安装终端的 `PATH` 以发现用户级 Agent 可执行文件，其中 systemd unit 会按其 quoting/specifier 规则转义）。兼容期 `/api/fault-injection/worker/*` 经 `worker-dual-auth.ts` 同时接受 API Key 与设备凭证，设备凭证路径下 `workerId` 恒等于 `clientId`。
...
- **RAS client install lifecycle**: platform `start` never installs or requires the Agent-host RAS runtime. `/api/ingest/setup` and `/api/ingest/setup/auto` install it when OpenCode, Hermes, or OpenClaw is selected, after persisting the current user's host/API key. RAS 使用三级制品回退（本地 checkout → `GET /api/ingest/setup/bundle?name=ras` → `npm pack`）；常驻客户端默认以当前服务端 `bundle?name=client` 为权威来源，再回退 `npm pack`，不会让执行目录中的旧 checkout 覆盖服务端修复。仅开发调试显式设置 `AGENT_INSIGHT_CLIENT_SOURCE=local` 时才运行 `./scripts/install-ras-client.js`，文件不存在即失败而不静默换源。bundle 路由从服务端安装目录打包白名单文件（含 `scripts/lib/fi-python-runtime.js`，不把 query 插入文件系统路径），所以仓库外、无 npm 环境也能安装。`AGENT_INSIGHT_CLIENT_PACKAGE_SPEC` 可把 npm 兜底改为 Agent 宿主可访问的包 URL。解压后运行 `scripts/install-ras.js`，再对 RAS ingest 路由发带鉴权的只读 GET 预检。`install-ras-client.js` 把 `reliability-client.cjs`、`ws-client.cjs` 和 `config_sync.js` 复制到 `~/.agent-insight/client/runtime/`，避免 systemd/launchd 指向已删除的 `/tmp` 解压目录；FI 运行时先安装验证、再刷新会撤销旧值的设备凭证，以缩短重绑窗口。普通组件从 `/api/ingest/setup/*` 下载，经临时文件落盘；OpenCode telemetry 不完整时报告 `PARTIAL`，不误报 `READY`。重启保留已有客户端 API Key，不用内部管理员 Key 覆盖。
...
- **Generated Trace experiment detail** — `POST /api/experiments/:id/run` 用 `traceSource=generate` 启动生成，并以 `fiOrchestrate` 显式区分执行通道：可靠性 Case 走 FI，普通评测 Case 走 `RUN_EXPERIMENT_CASE`。普通生成按 Case 下发结构化 `platform/agent/model/input/correlation`，客户端能力必须声明 `runExperimentCase.returnsTraceId=true`；OpenCode 输入以注册 Slash Command 开头时，客户端拆出命令名与参数并调用 `opencode run --command <name> <arguments>`，确保命令钩子、Agent 路由与 checkpoint Prompt 正常执行，普通 Prompt 则经 stdin 原样输入，两条路径均不拼 shell 字符串。客户端解析到平台 sessionID 后立即回 `RUNNING(state=TRACE_STARTED, traceId)`，进程退出后再回终态。服务端接受运行中或终态结果的 `traceId`，只按当前用户与 `Execution.taskId = traceId` 绑定 root Execution，不提供输入匹配兼容路径，并要求 Session `endTime` 非空且 `interactions` 是非空数组。客户端超时先终止整个进程组，5 秒后仍存活则强杀，避免 long-poll 队列被旧进程占用。每次执行记录为 `ExperimentTraceAttempt`，临时故障默认自动重试两次；自动恢复只检查当前生成周期中的 Attempt。客户端不接受自由命令字段。`GET /api/experiments/:id` 对 FI 或普通生成 Case 返回 `traceStatus: pending | ready | failed`、`traceError`、`traceAttemptNo` 与 `traceAttemptStatus`，并在顶层返回 `traceProgress { total, ready, failed, pending }`。只把 ready Case 调度给评估引擎，零 ready 时实验终态为 `failed`。`POST /api/experiments/:id/cases/:caseId/retry` 是 Case 操作列统一入口，按来源元数据分流：存在普通生成 Attempt/Command 或 FI Run 的 Case 始终清除旧绑定、开启新生成周期并在新 Trace 绑定后重跑全部评估器；没有生成元数据的已有 Trace Case 只重试失败评估结果。运行中的 Case 返回 409。
...
| `JudgeCaseContext` | interface | `src/lib/evaluators/judge-assembly.ts` | 自建 Judge 的 `input/datasetInput/output/referenceOutput/trajectory` 上下文；`datasetInput` 来自确定性匹配快照 |
