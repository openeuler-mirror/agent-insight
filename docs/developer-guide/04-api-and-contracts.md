...
- **Qoder plugin downloads**: `GET /api/ingest/setup/qoder-desktop-vsix` builds the Qoder CN Desktop VSIX from `integrations/qoder-desktop/`; `GET /api/ingest/setup/qoder-jetbrains-plugin` builds the JetBrains plugin ZIP from `integrations/qoder-jetbrains/`. Package names are derived from the source manifests, and completed packages are cached under `.next/cache/qoder-plugins/` until a source mtime changes. Desktop packaging is a deterministic ZIP operation. JetBrains packaging must first compile Java into a plugin JAR using `JETBRAINS_HOME`/the IDE JBR or Gradle `buildPlugin`; Java source files are never served as an installable plugin. Both routes use attachment filenames, disable MIME sniffing, and accept no caller-controlled filesystem path.
