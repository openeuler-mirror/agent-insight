...
- **RAS capability catalog**: `GET /api/agent-ras/catalog` returns detector domains + flattened submodes + JSON Schema / defaults from `agent_ras` `DETECTOR_PLUGIN.presentation` (spawn Python `build_capability_catalog`). UI catalog/configure consume this API; do not hardcode rows in TS. Defaults template: `agent_ras/config/agent_ras_config.default.yaml`. Design: `docs/agent-ras/designs/features/capability-catalog-decouple.md`.
- **Fault injection BFF / Worker** (`/api/fault-injection`): Insight 负责任务编排、展示与 Judge；本机 FI Worker 认领并执行注入。任务：`POST/GET /tasks`、`GET /task/:taskId`、`POST /tasks/stop`、`POST /tasks/delete`、`POST /task/:taskId/rerun`；Run：`GET /runs/:runId`、`GET .../trace`、`POST .../rejudge`；目录与健康：`GET /faults`、`GET /platforms`、`GET /platforms/:platform/agents|models`、`GET /health`、`GET /setup`（curl 安装脚本）。Worker：`POST /worker/heartbeat`、`POST /worker/claim`、`POST /worker/commands`、`POST /runs/:runId/collect-result`（写入 Prisma 后 **只读库** 再 Judge；`rejudge` 同；无 FI→RAS bridge；collect **不含** `injectionEvidence`）。鉴权：`x-witty-api-key` → user。`POST /tasks` 只落库 `queued`，由 Worker claim；无在线 Worker 时 platforms/health 引导安装。UI：`/agent-ras/fault-injection/tasks`（目录 `/faults`）。设计：`docs/agent-fault-injection/designs/modules/task-orchestration.md`、`docs/agent-fault-injection/designs/server-client-split.md`；用户指南：`docs/user-guide/observability/fault-injection.md`。
