...
- **LlamaIndex adapter**: `scripts/llamaindex_extension` 通过 dispatcher 输出 `service.name=llamaindex` 的 OTLP/HTTP JSON span。`src/lib/ingest/otel/llamaindex.ts` 负责来源识别，`src/lib/ingest/otel/adapters/llamaindex.ts` 按 `agent.instance.id` 重建 Agent 实例、同名并发与多级父子关系，并把 Tool、LLM、Retriever、Synthesizer、Workflow step 归一化为既有 `ExecutionRecord`/interaction 契约。npm 安装 Agent Insight 服务端并携带采集器源码；setup/auto setup 从只读 zip 路由取得运行时归档，直接部署到 `~/.agent-insight/collectors/llamaindex/current/`。`AGENT_INSIGHT_LLAMAINDEX_PYTHON` 指定的解释器只执行版本/依赖探测、标准库解压和 CLI 配置，不调用 pip。安装后 CLI 将 endpoint 与 API Key 写入 `~/.agent-insight/llamaindex.json`。
