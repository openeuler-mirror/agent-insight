...
- **Trace lifecycle**: `src/lib/observe/trace-lifecycle.ts` is the shared rule for every framework. Explicit `Session.endTime` wins; an AcTrail `agent-process-exit` failure with a valid end is `failed`, otherwise explicit completion is `success`. Without completion, 600,000 ms since `Execution.lastIngestedAt` produces `timed_out` / `inactivity-timeout`, regardless of framework or assistant output. Legacy null rows fall back to `Execution.timestamp`. `trace_completed_at` remains null for timeouts; `trace_last_received_at` exposes the timer basis. New ingest resumes `running`; metadata edits and replaying the same consumer batch do not advance this clock. The nullable column is added through Prisma schema sync and OpenGauss initialization. Lifecycle no longer uses the old auto-evaluation quiet-window allowlist. List and detail polling refresh status every 5 seconds; timeout polling continues for recovery.
- **Trace filter facets**: `listObservedFieldValues(column, user)` 按当前用户统计非空候选值；`subagentType` 使用 `isSubagent=true`，其他支持字段继续使用主执行。类型候选不随页面当前 Agent 范围隐藏，应用条件时仍与范围硬筛选组合。
...
- **AcTrail specialization**: AcTrail payloads are detected by `actrail.trace.id`, an `actrail.*` instrumentation scope, or `actrail.action.kind`. `normalizeActrailOtlpTraces` preserves semantic-action links and response usage. The adapter applies last-arrival-wins only to duplicate AcTrail Span IDs; pairs `llm.call` with request/response action IDs; reads the last real user message from `llm.request.canonical_body_json`; keeps a bounded, role-normalized projection of each call's canonical system/history/current-input messages; and projects `llm.tool_call`, `llm.tool_result`, `agent.invocation` plus typed Span Links into the common interaction and subagent-tree shape. Explicit `agent.name` / `agent.child.executable` identity remains authoritative. Next, `actrail.agent.type` on the root process `agent.identity` supplies `agentName` (`opencode` → `OpenCode`, `claude-code` → `Claude Code`, other nonempty strings unchanged). Identity is matched to the first root LLM call by trace and `actrail.process.id`; without process metadata, only an unambiguous identity is accepted. The raw `actrail.agent.type.source` remains in event attributes (`known_product` match versus `executable` fallback), and `framework` stays `actrail`. Generic `agent.identity` and `agent identity process-*` span names are ignored; recognized request User-Agent, command invocation and `AcTrail Agent` remain fallback paths. Late identity events update names on the next snapshot; historical records require retained events to be reprocessed. Tool results prefer an explicit typed Link or a unique `bound` tool-call ID. When AcTrail omits a standalone result action, the adapter also reads canonical request content blocks, binds `tool_result` / `function_call_output` through an exact unique call ID, and includes immediately following text blocks in that result; it never falls back to time proximity. Subagent ownership is propagated only through `agent.invocation.child_llm_request` and trajectory Links, never time proximity. Embedded `llm.response.tool_calls_json` remains the source for full arguments and the fallback for older payloads without standalone tool spans. The framework registers snapshot replacement so later snapshots replace partial projections instead of accumulating duplicate interactions. Lifecycle completion also consumes complete `process.exit` / `agent.exit` actions whose `actrail.process.id` exactly matches every root LLM process and whose `process.exit_code` is an integer. Nonzero exits persist an `agent-process-exit` entry in the existing `Execution.failures` field and `Session.endTime`; observe light/full reads and pagination report `failed`. Zero exits complete without synthesizing an answer. `command.invocation` success only confirms exec, so it is never a termination signal. Missing process IDs, unrelated child exits, unknown exit codes, and exits older than the latest root call do not supply completion. Existing records need their retained OTLP events to be reprocessed or a new event to arrive; records without exit evidence cannot be repaired by this rule.
