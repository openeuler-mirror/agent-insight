...
- **AcTrail raw capture**: after an incoming payload is identified as AcTrail and its normalized events have been appended to the normal session spool, the traces route synchronously preserves the received request under `~/.agent-insight/otel_data/traces/raw/actrail/YYYY-MM-DD/requests.jsonl`. Each row records the receive time, request ID, content type, encoding, SHA-256, derived session IDs and authenticated username. JSON requests retain their exact UTF-8 request text; protobuf requests retain the exact wire bytes as Base64. This is a temporary integration-diagnostics hook intended to be removed after AcTrail end-to-end validation. These rows are diagnostic input and are never discovered by the normal OTel consumer. An AcTrail response includes `rawCaptured: true` when the diagnostic copy succeeds or `rawCaptured: false` when only the normal trace spool append succeeds.
