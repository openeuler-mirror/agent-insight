...
- **Trace lifecycle**: callers that have an explicit completion signal pass `trace_completed_at`; `saveExecutionRecord` writes it to `Session.endTime`. Hermes records without an explicit value are allowed to infer completion from the latest interaction timestamp only when a final result is present. Observe pages derive trace execution status from `Session.endTime`, not from `final_result` or evaluation status.
