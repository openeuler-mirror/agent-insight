...
- **Trace normalization**: `src/lib/ingest/otel/decode.ts` converts transport payloads into the JSON-compatible OTLP shape. `normalizeClaudeOtlpTraces` maps common `gen_ai.*` / `llm.*` fields plus Hermes-style `llm.model_name`, `llm.token_count.*`, `input.value`, `output.value`, and `hermes.session_id`. `aggregateOtelTraceEvents` delegates Hermes sessions to `src/lib/ingest/otel/adapters/hermes.ts`; that adapter builds a `spanId` / `parentSpanId` tree and emits platform-shaped `Session.interactions` for user input, tool steps, intermediate `api.*` assistant output, and final assistant output. Hermes registers `sessionMergeStrategy: "snapshot-replace"` through `FrameworkAdapter`, so later partial OTLP batches replace the session snapshot instead of being monotonic-merged. Generic non-Hermes traces continue through the common OTel aggregator.
