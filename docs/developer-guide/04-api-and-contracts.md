...
- **LlamaIndex adapter**: `scripts/llamaindex_extension` 复用官方 `llama-index-observability-otel` 的 Handler 与 OTel span/context，使用隔离 `TracerProvider` 和 `SimpleSpanProcessor`；自定义 `AgentInsightSpanExporter` 只把结束的 `ReadableSpan` 转为 `SpanRecord` 并执行非阻塞入队，磁盘与 HTTP 均由后台线程处理。它输出 `service.name=llamaindex` 的 OTLP/HTTP JSON span，但不替换进程全局 Provider。`src/lib/ingest/otel/llamaindex.ts` 负责来源识别，`src/lib/ingest/otel/adapters/llamaindex.ts` 按 `agent.instance.id` 重建 Agent 实例、同名并发与多级父子关系，去除同一逻辑调用的 LLM 包装 Span，并把 Tool、LLM、Retriever、Synthesizer、Workflow step 归一化为既有 `ExecutionRecord`/interaction 契约。npm 安装 Agent Insight 服务端并携带采集器源码；setup/auto setup 在所选解释器中安装固定版本 `llama-index-observability-otel==0.6.4`，再从只读 zip 路由取得 Agent Insight 运行时归档并直接部署到 `~/.agent-insight/collectors/llamaindex/current/`。安装后 CLI 将 endpoint 与 API Key 写入 `~/.agent-insight/llamaindex.json`。卸载只移除 Agent Insight 专属源码、入口、配置与可选 spool，不卸载共享官方 OTel 包。
