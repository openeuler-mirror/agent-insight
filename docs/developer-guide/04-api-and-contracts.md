...
- **RAS capability config**: `GET/PUT /api/agent-ras/config?platform=` stores per-user×platform desired `AgentRASConfig` under `~/.agent-insight/data/ras-capability-configs/`. `GET /api/ingest/ras-config?platform=` is the client pull endpoint; when `syncEnabled` is false the payload omits `config`. OpenCode plugin merges on startup via `agent_ras/platform_adapter/opencode/config_sync.js` (fail-open; preserves `service.*`). UI: `/agent-ras/fault-modes?view=configure`. Design: `docs/agent-ras/designs/features/capability-config-sync.md`.
