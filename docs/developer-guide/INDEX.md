...
历史补充（2026-09-08，当时基于 358c0313；现已包含于上述提交）：统一实验 Agent/Skill 选择分离、Skill 加载确认协议、Agent 候选来源过滤及单 Case A/B 重跑，见 04/05。

历史补充（2026-09-08，当时基于 358c0313；现已包含于上述提交）：评估器对比第四步采用共享 A/B 分区组件，版本化两组分别多选，原生两组只读确认；存在、ready、上下文和互斥按组校验，开始按钮与 submit 使用同一门控。契约及控制流见 04/05；专项 122/122、构建与版本化真实规则执行及原生页面门控已验证，原生真实 LLM 尚未执行，全量仍有 47 项失败，详见现有测试报告。
