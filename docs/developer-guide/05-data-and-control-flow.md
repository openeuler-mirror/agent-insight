...
3. 调用 `normalizeClaudeOtlpTraces` 将 OTLP 数据归一化为内部 Event 格式；`witty.*`、标准 `gen_ai.*` 与已有兼容别名在这里收敛
