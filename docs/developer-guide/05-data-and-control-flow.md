...
评测通信配置由 `EvaluatorRuntimeConfigProvider` 统一提供：每次相关操作从 `data/config/benchmark-evaluator.env` 读取一份 URL、当前 Token、宽限期 Token 与 HTTP 策略的完整快照，文件缺失时回退进程环境变量。合法原子替换在下一次操作生效，非法或半写入更新继续使用上一份有效配置。`AGENT_INSIGHT_PUBLIC_BASE_URL` 是冻结到实验绑定、供远端 Evaluator 回调的平台公开地址；可选的 `AGENT_INSIGHT_BENCHMARK_EXECUTOR_CALLBACK_BASE_URL` 只覆盖客户端执行期间回调平台所用的地址，未配置则沿用公开地址。Benchmark 执行任务本身通过现有客户端 WSS/长轮询控制通道下发，因此这个变量不是执行器地址，也不要求客户端开放端口；客户端只校验回调 URL 的 HTTP(S) 协议和精确 Run 路径。执行 Outbox 冻结实际执行回调 URL，Evaluation 继续从实验绑定读取公开地址；启动恢复只对尚未生成 Outbox 的 Case 读取当前覆盖值，已冻结任务不会被改写。新 Evaluation 同时冻结目标 URL 与发送凭证修订，避免切换期间拼接新旧值；已冻结旧目标的重试不会自动拿新 Token 请求旧地址。Linux/macOS 上由 `start-evaluator.sh` 构建和常驻运行 Controller；构建期 Debian/PyPI 默认使用国内镜像并在失败时回退官方源，SWE-bench Harness 从官方 GitHub codeload 下载固定 commit archive 并校验固定 SHA-256。Node 基础镜像名称保持官方值并复用宿主 Docker daemon 的 registry mirror；Case 镜像名称同样保持官方值，本地不存在时先通过可配置的 `SWE_BENCH_IMAGE_PROXY_PREFIX` 拉取并恢复官方 tag，代理失败再回退 Docker Hub。在线镜像优先冻结 registry digest；`docker save/load` 离线导入且没有 `RepoDigests` 时冻结不可变 Image ID。部署脚本不修改宿主全局配置。默认 Doctor 不拉取 Case 镜像，显式 Gold Smoke 和真实任务才按需拉取。
