...
AcTrail 已由用户独立安装并通过 `actrailctl launch` 启动 Agent，Agent Insight setup 不安装或包装 AcTrail。Unix/WSL setup 为现有 AcTrail 官方 `otel-http` 插件生成带当前用户 `x-witty-api-key` 的完整属性配置，通过 `actraild plugin load --persist` 加载后，AcTrail 直接向 `/api/ingest/otel/v1/traces` 上报 protobuf。接收端先同步写正常的 session 分片 spool；在联调阶段还会把已识别为 AcTrail 的原始请求同步复制到独立 raw 目录，供字段和缺失问题复盘。AcTrail adapter 按动作关系合并 `llm.call/request/response`，并将工具、Skill、子 Agent 与 token 投影为公共 `ExecutionRecord`；重复 span 按最后到达版本覆盖。
...
OTel spool 新写入按 day + session 分片：ClaudeCode logs 使用 `otel_data/claude/YYYY-MM-DD/sessions/<safe-session>/logs.jsonl`，CodeAgent logs 使用 `otel_data/codeagent/YYYY-MM-DD/sessions/<safe-session>/logs.jsonl`，Hermes/通用 traces 使用 `otel_data/traces/YYYY-MM-DD/sessions/<safe-session>/traces.jsonl`。Consumer 递归发现 JSONL shard，并继续兼容旧的 `YYYY-MM-DD/logs.jsonl` / `YYYY-MM-DD/traces.jsonl` 日文件。临时联调口子另将 AcTrail 原始 HTTP 请求写入 `otel_data/traces/raw/actrail/YYYY-MM-DD/requests.jsonl`；该目录不在 Consumer 的数据源中，不会重复入库，完成端到端验收后应删除此旁路。
...
3. 调用 `normalizeOtlpTraces` 将 OTLP 数据归一化为内部 Event 格式；`witty.*`、标准 `gen_ai.*`、`actrail.*` 与已有兼容别名在这里收敛
4. `appendOtelTraceEvents` 先将规范化 events 写入按 session/trace 分片的正式 JSONL spool；若识别为 AcTrail，再把收到的原始 JSON 文本或 protobuf 字节同步复制到临时诊断 raw spool
5. 返回 `{ status: 'accepted', received, sessions }`；AcTrail 响应另带 `rawCaptured`，`true` 表示诊断副本已同步落盘，`false` 表示正式数据已受理但诊断副本写入失败；两者都不表示后台 Consumer 已完成持久化
