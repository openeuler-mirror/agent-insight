...
统一 `agent-debug-diagnosis` Skill 承载三条路线：一键诊断继续运行 AgentDebug 五模块；普通追问保持现有上下文问答；定向查因只在用户症状命中诊断器清单时追加对应专项结果，不启动五模块。专项诊断器位于 `skills/agent-debug-diagnosis/detectors/<name>/`，每个目录通过自己的 `detector.json` 自注册，公共 `scripts/detector_runner.py` 扫描、匹配和执行；服务端仅提供通用运行、富化和报告适配，不维护诊断器名称注册表。

一键诊断分为两个顺序阶段。第一阶段只向 AgentDebug 提供 Trace、静态检测和五模块规程，生成并冻结 `coreFindings`，不暴露专项诊断结果；第二阶段复用同一 AgentDebug 会话，只输入冻结的 `coreFindings` 与经过通用富化的 detector findings，并要求它只返回 `merge` / `independent` 决策。`finding-reconciler.ts` 以确定性代码应用决策：任何 core finding 及其标题、证据和修复建议都不会被删除或改写，第二阶段只能提升优先级；无效或缺失决策默认保留独立 finding，计数、区间、比例、锚点和 `details` 从原始 detector finding 无损复制。重复结果写入目标 finding 的 `supplementalEvidence`，独立结果保留在通用 `detectorFindings`；前端统一渲染且不展示诊断器来源标签。原始结果和请求/实际应用的决策只写入诊断工作区的 `.agent-insight/` 审计文件。
