...
客户端 agent（OpenCode 插件 + uploader、Claude Code 官方 OTel logs、Hermes `hermes-otel` 插件、OpenClaw watcher、OTel SDK）将运行数据推送到接入路由。平台将原始 session 规范化为一棵 `Execution` 树。Claude Code 的 `tool_result` log 只包含工具名、输入和结果大小等 metadata；工具输出正文从 raw API request body 的 `tool_result` blocks 回填，因此安装脚本将 `OTEL_LOG_RAW_API_BODIES` 配成 `file:<dir>`，避免 inline `1` 模式被 Claude Code 截断到 60 KB。`OTEL_LOG_TOOL_CONTENT=1` 只影响 tracing span events，需要启用 traces。Hermes 由 setup 脚本安装并启用 `briancaffey/hermes-otel`，路径探测以 `$HERMES_HOME` 为中心：默认使用 `$HERMES_HOME/hermes-agent/venv/bin/pip` 安装依赖和 editable plugin，插件配置写入 `$HERMES_HOME/plugins/hermes_otel/config.yaml`；fallback 只探测 `~/git/hermes-agent/venv` 与 `~/agent/hermes-agent/venv` 这类手动 clone 形态。配置会开启 `capture_full_responses`、`capture_conversation_history` 与较大的 `preview_max_chars`；不改动原 Hermes 工具代码时，工具输出仍受插件采集能力限制。OTel `logs` / `traces` 是异步摄取：HTTP 端点只负责解码（traces 支持 OTLP http/json 和 http/protobuf）、校验、归一化、写 JSONL spool 并返回已受理；`instrumentation-node.ts` 启动的进程内 `OtelSpoolConsumer` 再按 checkpoint 增量消费、短 debounce 快速落库、长 debounce 触发评估。OTel traces can branch through framework-specific trace adapters before storage normalization: Hermes uses `src/lib/ingest/otel/adapters/hermes.ts` to rebuild the `spanId` / `parentSpanId` tree and emit platform `Session.interactions` for user input, tool steps, intermediate api-span assistant output, and final output; non-Hermes traces still use the common OTel span aggregator. Hermes 的 `FrameworkAdapter.sessionMergeStrategy` 是 `snapshot-replace`，同一 session 的后续 OTel batch 会以最新完整 snapshot 覆盖旧 interactions，而不是走通用 monotonic merge。
...
```
关键函数：接入路由处理器（`processUploadAsync`、`proxyFetch`、OTel `POST`）→ OTel traces 路由的 `decodeOtlpRequest` → `normalizeClaudeOtlpLogs` / `normalizeClaudeOtlpTraces` + spool append → `otel-consumer/consumer.ts:startOtelSpoolConsumer` / `runOtelSpoolConsumerTick` → `src/lib/ingest/claude-otel/traces-aggregator.ts:aggregateOtelTraceEvents`（Hermes 分流到 `src/lib/ingest/otel/adapters/hermes.ts`）→ `src/lib/ingest/adapters/registry.ts:getAdapter` / `storage/data-service.ts:extractInvokedSkillsFromSessionInteractions` → `src/lib/ingest/*` + `engine/observability/{claude,openclaw}-parser.ts`、`agent-trace.ts:buildAgentCallTree` → `storage/data-service.ts:saveExecutionRecord` / `deriveSubagentExecutions`。当前 adapter 只承担纯转换职责（skill 抽取、Claude 存储归一化、框架名解析、OTel trace 形状适配），不写库。
