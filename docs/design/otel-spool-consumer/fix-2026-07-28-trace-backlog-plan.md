...
### 4.3 存量积压 + 部署重启（上线最关键的一步）

场景构造：先正常跑一小段（产出一份**已有 checkpoint**，模拟"在运行中的老实例"），停服，然后在
consumer 不在的情况下直接往 spool 里塞 **3000 个新格式分片会话 + 一个 60MB 旧格式整日文件
（22392 行 / 420 个会话，其中 20 个跨格式边界）**，spool 共 104MB，再启动。

结果：

| 观察项 | 实测 |
|---|---|
| 存量会不会被跳过 | **不会**。`seedToEof` 只在**没有 checkpoint 时**才把游标推到 EOF；老实例有 checkpoint，存量会被完整重放 |
| 启动后多久开始出数 | 2 秒内已入库 952 条（backlog ≥200 立刻进排空模式） |
| 端点是否还收得进上报 | **正常**。全程 2 trace/s 探针 299 发 299 成功，HTTP P95 **5ms** |
| backlog 走势 | 3001 → 0 |
| 数据完整性 | 3380 分片会话 + 400 legacy-only 会话 = **3780，与库中行数完全一致，零丢失** |
| 再重启一次 | backlog 直接为 0、**没有任何重读**、行数仍是 3780、**零重复**（task_id upsert 幂等） |

> ⚠️ **全新安装要注意**：`seedToEof` 在没有 checkpoint 时会把当前 spool 全部标记为已消费。
> 也就是说，把已有 spool 目录挂到一个**从未跑过 consumer** 的新实例上，这批存量会被**直接跳过**。
> 这是既有设计（避免首次启动重放整个历史），不是本次改动引入的，但迁移/换机时必须知道。

**这一轮抓出两个本次改动引入的真 bug**（都已修复并补了回归用例）：

1. **evaluated 路径漏做文件归属簿记**。存量积压下成百上千个会话在第一个 tick 全被发现，长 debounce
   内根本处理不完，于是 fast 与 evaluated 双双到点、dispatcher 直接跑 evaluated 跳过 fast ——
   而 `markSourceDone` 只在 fast 分支调用。后果：`pendingFiles` 卡住不降、**checkpoint 游标永不推进**，
   这些 spool 文件每次重启都要重读，retention 也永远归档不掉。实测 backlog 卡在 520 不动。
2. **多会话文件完成后只回收当前会话**。旧格式整日文件一个文件就有几百个会话，文件完成时其余会话
   对象永久留在内存（实测 `sessions` 长期停在 420 不降）。最小复现里 100 个会话跑完仍剩 44 个。

**一个未收敛的观察**：第一轮（修复 ① 之前）出现过 `backlog` 停在 `1` 不降。当时核对过账目 ——
数据一条不少、checkpoint 全部到 EOF、重启后即归零，属于内存簿记残留而非数据问题。
用最小复现脚本单独构造未能重现，怀疑与真实运行时的时序有关。**已知无数据影响，但结论未闭环**。

### 4.4 第二个瓶颈：evaluated 阶段同步等结果质量评估（2026-07-30 现场定案）

前面三节的修复上线后，同事一度确认"问题消失了"，随后又复现。**代码没有被回退** —— 逐字节比对过
`upstream/master`（`b72ffc0`）与本分支的五个生产文件完全一致。真相是：①②③ 把读路径与限流这个瓶颈
移走之后，另一个一直存在的瓶颈变成了唯一瓶颈。

**生产现场证据**

| 观察项 | 实测 |
|---|---|
| `stalled dispatch` | 2 小时 119 条、1 小时 60 条。该日志**全局每 60 秒最多一条**，即几乎每一分钟都有任务卡 ≥15s |
| 同一 sessionId 的 `ageMs` | `136939 → 257003 → 317033 → 377067 → 437096` —— 一个会话独占调度器 7 分钟以上 |
| 单轮 `costMs`（9 样本 / 2 小时） | 最小 1815ms、**中位 624610ms（10.4 分钟）**、P95 1550412ms、**最大 1747156ms（29.1 分钟）** |
| CPU | **7～8%** —— 不是在算，是在等 |
| 裁判模型配置 | dev/beta 端点，且 `apiKey` 个数 = 0（代码 `apiKey: config.apiKey \|\| 'no-api-key-required'`，没凭据也照发） |
| 裁判单次调用耗时 | 现场确认 **约 3 分钟** |

**机制**：`consumer.ts` 的 evaluated 阶段同步 `await scheduleResultEvaluation`，而该调用会
① 抢 `withBackgroundOpencodeSlot` 的槽位（默认 5 个，与技能生成/实验评测共用）
② 跑 `RESULT_METRIC_KEYS` 的 4 个维度（`faithfulness / instruction-adherence / answer-quality / accuracy`）。
四个维度是 `Promise.all` 并行的，但 `accuracy` 内部是**串起来的两段**（先 `matchAgentDatasetCase`
语义匹配、再发裁判调用），所以单条 evaluated 的下限就是两次串行调用。

**致命之处在于单飞**：调度器一次只允许一轮（为保 SQLite 单写），所以一条 trace 的评估堵住的不是它自己，
**是所有人 —— 包括刚上报进来的新 trace 的 fast save**。这就是"新数据看不到"的确切机制。
吞吐上限 ≈ 6 条/小时，与实测 9 次慢轮 / 2 小时吻合。

**为什么之前"消失了"**：fast save（3s，`skip_evaluation`）不碰评估，一直很快；只有 evaluated save
（30s）走评估。修复前所有环节都慢，这个瓶颈被埋着；修复后读路径从 405ms 降到 0.24ms，它占满了 100% 的墙上时间。

**为什么 §4.1/§4.2 的压测没测出来**：隔离环境没配裁判模型，`createClient()` 直接 `return null`，
评估立刻返回。这是原验证方案的真实缺口。

**修复**：evaluated 阶段改为 fire-and-forget（`void ... .catch(...)`）。理由很硬 —— 结果质量评估
本身就是设计成后台任务的（它自己要过槽位限流），consumer 没有任何理由同步等它。

**复现与验证**（假裁判端点每次调用固定延迟 180s，20 trace/s × 30s = 600 条，观察 480s，两轮同参数）

| 时刻 | 修复前（同步 await） | 修复后（fire-and-forget） |
|---:|---:|---:|
| 25s | 400 | 399 |
| 45s | **539** | **600** |
| 105s | 539 | 600 |
| 245s | 539 | 600 |
| 480s | **539（7 分钟一动不动，61 条永不入库）** | **600（全部入库）** |

| 其他 | 修复前 | 修复后 |
|---|---:|---:|
| `stalled dispatch` | 8 | **0** |
| `slow aggregate` | **一条都没打**（轮次在整个窗口内从未结束） | — |
| 评估是否照常触发 | 是 | **是**（假裁判收到 60 次调用，在途 20 = 5 槽 × 4 维度） |
| `TraceEvaluation` | — | 20 行（10 done / 10 running，后台并发跑） |
| CPU 均值 | 9% | 10% |

关键点：修复**不是把评估关掉** —— 评估照常触发、照常在后台并发执行，只是不再决定 trace 何时可见。

**回归用例可判别性**：新增用例「结果质量评估永久 pending 时，consumer 必须继续处理后续 trace」
在未修复的 `upstream/master` 上**失败**（"实际只入库 1/6 条"），在修复后 6/6 通过。
