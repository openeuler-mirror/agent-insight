...
### A.1 顶层布局（v2.1 · 2026-05-27 重设计）

```
┌──────────┬───────────────────────────────────────────────┐
│ Sidebar  │  <PageContainer>                              │
│ (w=220,  │    PageHeader      (h=56, sticky, z=10)       │
│  0 折叠) │    PageToolbar                                │
│          │    PageContent                                │
│  [Brand] │    PageFooter                                 │
│  [Nav]   │                                               │
...
- **PageHeader 高度 56px**（v1.x 的 AppTopBar 48 + PageHeader 自由高度，现在合并为单行 56px；详情页用双行 36+56=92px 变体，见 §A.5.3 `detail-object`）。
- **Sidebar**：默认展开 220px、折叠后**宽度归 0**（不保留 56px 缩略条 —— 折叠态由 PageHeader 左侧的 `sidebar-toggle` 接管展开操作）。一级导航 ≤ 7 项，超过下沉到二级。
...
- 🚫 **禁止重新引入 AppTopBar**。任何 PR 新增 `<AppTopBar>` / 顶部全局横条 —— CR 直接打回。如有"全站通知 / 系统消息"需求，走右下 Toast（Sonner）或 Sidebar 底部 BannerSlot，而不是新加顶部行。
...
PageHeader 是用户访问每个页面**第一眼看到的元素**，理应是产品识别度的高密度承载点。但按 §0.1 第 8 条 / §0.2 原则，**不允许**通过装饰元素来"加风格"。PageHeader 的签名来自四处**功能即风格**的细节：
...
3. **主操作永远在标题行右上**：不论 management / detail / live / detail-object 变体，标题行右侧是固定的"行动锚点"。用户在任意页面把视线扫到右上都能找到"我现在能做什么"。
4. 🆕 **Sidebar-toggle 永远是 PageHeader 最左侧第一个元素**：28×28px 透明按钮 + lucide `<PanelLeftClose>` / `<PanelLeftOpen>`（折叠态切换图标）。它不属于"装饰"，但它的**固定位置**让用户在任何页面都能用同一个肌肉记忆收起/展开侧栏 —— 这是 v2.1 把 AppTopBar 收编后唯一新增的统一锚点。

> 这四条之外**禁止**给 PageHeader 加任何装饰（渐变下划线、彩色徽章、背景图、阴影特效）。签名的本质是"在所有页面**完全一样**"，而不是"做得多漂亮"。

#### A.5.2 七个固定槽位（自上而下 · v2.1 起新增槽位 ⓪）

```
┌─────────────────────────────────────────────────────────────────────┐
│ ⓘ ⓘ row-1（h=56，单行变体）/ 双行变体的 row-1（h=36，crumbs 行）       │
│ ⓪ sidebar-toggle  ① eyebrow                                           │
│ [≡] Skills  ›  管理本地与企业 Skill                                    │  28×28 button + text-xs muted, ›=primary/50
│                                                                     │
...
| 槽位 | 必填 | 内容契约 |
| --- | --- | --- |
| 🆕 ⓪ sidebar-toggle | 必填（由 `<PageHeader>` 自动渲染，不传 prop） | 28×28 透明按钮 + lucide `<PanelLeftClose>` / `<PanelLeftOpen>`（折叠态切换）；`aria-label="折叠侧栏"` / `"展开侧栏"`；位于 PageHeader 最左 |
| ① eyebrow | 必填 | `<Breadcrumbs>` 或 `<ModuleLabel>`，至少 1 项；首页除外。**单行变体**里 eyebrow 与 sidebar-toggle 同一行（toggle 右侧 12px 起）；**双行变体**里 eyebrow 独占 row-1（36px 高，背景 `--surface-alt`） |
| ② title-row 左 | 必填 | 可选 16px 模块图标（lucide-react，`currentColor`）+ 标题文本 + 可选 inline `<StatusBadge>` / `<VersionTag>`。`detail-object` 变体下此处升级为"对象卡"（见 §A.5.3） |
| ② title-row 右 | 视变体 | 见 §A.5.3 四种变体 |
| ③ description | 选填 | ≤ 2 行；超 2 行用 `<ExpandableText>`。`detail-object` 变体下不出现（对象副信息已由 row-2 的 `context-sub` 承担） |
...
#### A.5.3 四种变体（title-row 右侧 · 互斥）

| variant | 行数 | title-row 右侧 | 适用模板 | 示例 |
| --- | --- | --- | --- | --- |
| `management` | 单行 (56px) | 1 个 `default` Button（主操作），可选 1 个 `secondary` Button（如"同步"） | 模板 1 列表页 / 模板 3 表单页 | `+ 注册 Agent`、`+ 上传 Skill`（可选 ↻ `同步`） |
| `detail` | 单行 (56px) | 1 个 `default` Button + 1 个 `ghost icon` 的 `⋯` Overflow Menu | 模板 2 详情页（轻量，无对象身份强调） | `Run` + ⋯（包含 Edit / Duplicate / Delete） |
| `live` | 单行 (56px) | 实时集群：● 实时徽章 + 时间戳 + 刷新频率 + 暂停/继续按钮 | 模板 4 Dashboard / 模板 5 流式页 | 概览 / Trajectory |
| 🆕 `detail-object` | 双行 (36+56=92px) | row-1 = crumbs；row-2 = `[对象 icon] 标题 + 版本下拉 [chev] + 副信息 ... [secondary action] [default action]` | 模板 2 详情页（**对象详情** —— 页面身份围绕 1 个具名实体） | Skill 详情 / Skill 分析 / Trace 详情 / Run 详情 / Agent 详情 |

> **何时用 `detail-object` 而不是 `detail`**：判断准则是"这个页面去掉对象名后还成立吗"。如果页面的全部内容都在围绕某个具名对象（一个 Skill 包、一个 Trace、一次 Run）展开 —— 标题就是对象名、版本切换是高频操作、用户需要时刻看见"我在看哪个 Skill 的 v3" —— 用 `detail-object`。如果只是"某个表的某一行"轻量详情（如"模型注册详情"、"用户详情"），用 `detail`。

##### `variant="detail-object"` 双行结构（row-1 + row-2 的精确组成）

```
┌───────────────────────────────────────────────────────────────── row-1: h=36 ──┐
│ bg=var(--surface-alt)  border-b=var(--border)  padding=0 24px                  │
│ [≡] Skills › 分析                                                              │  text-xs, breadcrumb-item.current = text-2 (非主色)
├─────────────────────────────────────────────────────────────── row-2: h=56 ────┤
│ bg=var(--surface)  padding=0 24px                                              │
│ ┌──────────────────────────────────────────────────────────────────────────┐  │
│ │ [L] linux-disk-cleanup [chev▾]            ……         [⤓ 导出] [▶ 主操作] │  │
│ │     v0 (当前) · 05-20 17:06                                                │  │
│ └──────────────────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────────────────────┘
```
...
| row-2 子元素 | 规格 | 备注 |
| --- | --- | --- |
| **对象 icon** (`context-icon`) | 32×32px，`rounded-md` (6px)，bg=`--primary-subtle`，fg=`--primary`，`font-mono` 12px **取对象名首字母大写** | 与 §0.2 锚点③（等宽数字单位对位）同源 —— 单字符 mono 强化"这是一个 ID 化的对象"。**禁止**塞 emoji / lucide 图标，**只允许**单字母。 |
| **对象标题** (`context-title`) | `text-base` (15px) `font-semibold` `letter-spacing -0.01em` `font-mono`；右侧紧贴 lucide `<ChevronDown size=12>` | mono 字体让对象名（多为 kebab-case `linux-disk-cleanup`）等宽对位；chev 触发版本切换 Radix Popover（见 `versionMenu` prop） |
| **对象副信息** (`context-sub`) | `text-xs` (11px) muted；格式 `{version} ({tag}) · {relativeOrAbsTime}` | 例：`v0 (当前) · 05-20 17:06` |
| **secondary action**（选填） | shadcn `<Button variant="ghost" size="sm">` + lucide icon | 例：`⤓ 导出` / `复制链接` |
| **default action**（必填） | shadcn `<Button variant="default" size="default">` + lucide icon | 主操作动词，例：`▶ 前往 Skill 优化器` |

> 🚫 row-2 **禁止**塞 ⋯ Overflow Menu。`detail-object` 把"次要操作"分成两类：
> - 直接相关于"对象本体"的（导出 / 复制链接）→ row-2 secondary slot。
> - 与对象同生命周期但更重的操作（删除 / 归档 / 转让所有权）→ 进入页面 body 内的 Settings Tab，**不**塞 ⋯。
> 这样做的代价是 row-2 操作位最多 2 个 Button，强迫产品做减法。
...
export interface PageHeaderProps {
  ...
  // 槽位 ② title-row 右（互斥四选一）
  variant: 'management' | 'detail' | 'live' | 'detail-object';

  // management / detail / detail-object 共用主操作
  action?: {
    label: string;
    icon?: LucideIcon;
    onClick: () => void;
  ...
  // management 选填次操作（如"同步"），detail-object 选填次操作（如"导出"）
  secondaryAction?: {
    label: string;
    icon?: LucideIcon;
    onClick: () => void;
  ...
  // detail-object 专属（与 title / icon / badges 互斥 —— 传 contextObject 时这三个 prop 走 row-2 而非 row-1）
  contextObject?: {
    iconChar: string;                      // 单字符大写（取对象名首字母）
    title: string;                         // 对象名，font-mono 渲染
    sub: string;                           // 例: "v0 (当前) · 05-20 17:06"
    versionMenu?: {                        // 版本切换下拉（标题旁 chev 触发）
      current: string;                     // 例: "v0"
...
**Case D · Skills 分析（`variant="detail-object"`）· v2.1 新增**

直接对应 [`refs/ui-2026-05/agent-insight-prototype-统一导航栏和skill选项.html`](./refs/ui-2026-05/agent-insight-prototype-统一导航栏和skill选项.html) 中 `data-page="analysis"` 的双行 header。

```tsx
<PageHeader
  breadcrumbs={[{ label: 'Skills', href: '/skills' }, { label: '分析' }]}
  variant="detail-object"
  contextObject={{
    iconChar: 'L',
    title: 'linux-disk-cleanup',
    sub: 'v0 (当前) · 05-20 17:06',
    versionMenu: {
      current: 'v0',
      versions: [{ id: 'v0', label: 'v0 (当前)', current: true }],
      onSelect: (id) => router.push(`/skill-detail/${skillId}?version=${id}`),
    },
  }}
  secondaryAction={{ label: '导出', icon: DownloadIcon, onClick: exportSkill }}
  action={{ label: '前往 Skill 优化器', icon: PlayIcon, onClick: gotoOptimizer }}
/>
```

`detail-object` 一次性覆盖原型里 Skills 分析 / Skills 详情 / Skill 历史 / Skill 优化结果等**所有 5 个 `data-page` 共享同一个双行 header 结构**的页面 —— 对象 icon 字符、title、版本下拉、secondary/primary action 一一对应。

#### A.5.6 红线（CR 直接打回）

- ❌ 页面自己用 `<h1>` + flexbox 拼 header（哪怕"看起来一样"）
- ❌ 主操作 inline 写在标题旁（如 Skill 管理 v1.2 的 `<h1>Skill 管理 <Button>+ 上传</Button></h1>`）
- ❌ 🆕 重新引入 AppTopBar 或任何"全局顶栏"行（v2.1 起 sidebar-toggle / 面包屑 / 主操作 / 实时集群 / 用户菜单 全部进 Sidebar 或 PageHeader，**不再有独立顶栏**）
- ❌ 把"实时刷新"/"暂停"按钮放在 PageHeader 以外的任何位置
...
- ❌ 🆕 `detail-object` row-2 塞 ⋯ Overflow Menu（次要操作只允许 1 个 secondary Button + 1 个 default Button，重操作走 Settings Tab）
- ❌ 🆕 `detail-object` 对象 icon 用 emoji / lucide 图标（**只允许**单字母大写 + `font-mono`）
- ❌ 🆕 把 sidebar-toggle 渲染在 PageHeader 之外（如挂在 Sidebar 自身、挂在内容区） —— toggle 必须由 `<PageHeader>` 自动渲染
...
#### A.5.8 外部依赖与库映射（实现 PageHeader 时用什么）

> **原则**：PageHeader 的所有子元素**必须**复用项目已装的库，**禁止**为 PageHeader 引入新 NPM 依赖。下表把每个槽位 / 子元素映射到具体的实现来源 —— 实现 `src/components/shell/PageHeader.tsx` 时，按表里"用什么"一列直接 import 即可。

| 槽位 / 元素 | 用什么（已装） | 备注 |
| --- | --- | --- |
| ⓪ sidebar-toggle | `<Button variant="ghost" size="icon" className="size-7">` + lucide `<PanelLeftClose>` / `<PanelLeftOpen>` | shadcn Button + lucide-react 都已装 |
| ① eyebrow → Breadcrumbs | **新建** `src/components/ui/breadcrumb.tsx`（基于 shadcn-ui 官方实现，零依赖，纯 a + nav 语义） | 项目已装 `@radix-ui/react-slot` `class-variance-authority` `clsx` `tailwind-merge` —— 满足 shadcn Breadcrumb 的全部依赖 |
| ① eyebrow → 分隔符 ` › ` | 写死字符串 `›` (U+203A) + `text-primary/50` | **禁止**用 lucide `<ChevronRight>`（图标比字符更重） |
| ② title-row 左 → 模块图标 | lucide-react `<LayoutDashboard>` / `<Bot>` / `<Package>` … | `size-4` (16px) `text-current` |
| ② title-row 左 → 标题 | 原生 `<h1 className="text-2xl font-semibold tracking-tight">` | 不引入额外库 |
| ② title-row 左 → 内联 badge | 复用 `<StatusBadge>` / `<VersionTag>`（已在 §Q.1 注册表） | — |
| ② title-row 左 → contextObject icon | 原生 `<div>` + Tailwind tokens (`bg-primary-subtle text-primary rounded-md font-mono`) | 单字符不需要 shadcn Avatar |
| ② title-row 左 → contextObject 版本下拉 | Radix `<DropdownMenu>`（已装 `@radix-ui/react-dropdown-menu`） + lucide `<ChevronDown size={12}>` | shadcn-ui dropdown-menu 已可直接套用 |
| ② title-row 右 → primary action | `<Button variant="default" size="default">` + 可选 lucide icon | — |
| ② title-row 右 → secondary action | `<Button variant="secondary" size="default">` + 可选 lucide icon | — |
| ② title-row 右 → detail variant 的 ⋯ | Radix `<DropdownMenu>` trigger=`<Button variant="ghost" size="icon">` + lucide `<MoreHorizontal>` | — |
| ② title-row 右 → live 实时徽章 | `<StatusBadge status="running">` (`<span className="size-1.5 rounded-full bg-current animate-pulse">`) | §0.2 锚点①已规定的呼吸 dot |
| ② title-row 右 → live 时间戳 | `<time className="font-mono tabular-nums text-xs">` + `date-fns` `format()` | `date-fns` 已装 |
| ② title-row 右 → live 刷新频率 | Radix `<DropdownMenu>` trigger 显示当前频率（`3s` 等） | — |
| ② title-row 右 → live 暂停按钮 | `<Button variant="secondary" size="sm">` + lucide `<Pause>` / `<Play>` | 文案切换：`暂停` ↔ `继续` |
| ③ description | 原生 `<p className="text-sm text-foreground-secondary max-w-[72ch]">` | 超 2 行套 `<ExpandableText>` |
| ④ hairline | 原生 `<div className="border-b border-border">` | — |
| ⑤ banner | shadcn `<Alert>`（**待建**，基于已有 token + Radix 风格） | 复用 sonner 触发的不是 banner —— banner 是页面内常驻 |
| ⑥ meta-strip → kpi | 自管 grid 容器，每项内部数值走 `<MetricValue>`（已在 §Q.1） | — |
| ⑥ meta-strip → filter | 复用 `<FilterBar>` (`src/components/filter/FilterBar.tsx`，已在 §Q.1) | — |
| ⑥ meta-strip → tabs | shadcn `<Tabs>` underline 变体 + nuqs `useQueryState('tab')` | nuqs 已装 |
| 主题切换 / 语言切换 / 用户菜单 | 留在 Sidebar Footer（v2.1 起从顶栏迁出 —— 见 §A.1） | 实现已存在于 `AppSidebar.tsx` |
| 全局搜索 Cmd+K | 全局键盘快捷键 + Sidebar Footer 入口；P2 引入 `cmdk` 包 | 见 §A.4 命令面板 |

> 🚫 **禁止引入**：`react-headroom`（PageHeader 不需要自动隐藏）、`@floating-ui/react`（Radix 已经覆盖所有 popover/dropdown 需求）、任何独立的 breadcrumb 包（shadcn 实现已足够）。
