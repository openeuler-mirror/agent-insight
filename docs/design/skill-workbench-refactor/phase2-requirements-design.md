...
- 空会话从左侧入口选择后，原子写入固定 Skill 和起始工作版本。
- 顶部资产选择器控制右侧四个页签；详情、正式评估、实验和优化记录均以所选 Skill + 版本为准，不修改或创建左侧过程会话。重新打开历史会话、生成或优化时恢复该会话保存的工作版本。
- 会话内发布只推进工作版本，不允许静默改绑其他 Skill。
...
生成和优化调用快照层；未发布上传快照通过工作台任务启动，管理中心正式版本直接按资产运行，不创建工作台会话或任务。进度至少包含：读取校验、结构与描述、流程与安全、汇总结果。最终结果仍落 `Evaluation + SkillIssue`，并绑定版本或内容 hash。
...
- 正式 Skill 实验直接绑定 `skillName + version`，不要求或自动创建工作台会话；只有由优化候选发起的兼容复测才可附带来源会话。
