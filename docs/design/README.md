...
| Agent RAS 可靠性闭环（服务端与客户端交互） | [`../agent-ras/designs/features/Agent RAS 可靠性闭环-服务端与客户端交互设计.md`](../agent-ras/designs/features/Agent%20RAS%20可靠性闭环-服务端与客户端交互设计.md) | 常驻客户端 + configRef 控制面；Trace 异常维度；可靠性数据集/评估器闭环；能力默认关闭 | Feature | 2026-08-11 | ⬜ 未实现（需求设计） | — |
