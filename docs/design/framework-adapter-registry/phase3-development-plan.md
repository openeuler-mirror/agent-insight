...
**安全网怎么搭的** —— T1 先建 golden 基线(把现状钉成"标准答案"),之后每搬一处都拿它对:结果一变,立刻知道改坏了。所以本重构的"行为不变"不靠人肉 review,靠测试机器证。
...
- [ ] **T1 golden 基线（先于一切搬迁）** — `test/framework-adapter-golden.test.ts` + `test/fixtures/framework-skill-fixtures.ts`
  ...
  - **不许做**:不改任何被测函数。
  - **怎么算过**:三框架抽取与 claude 归一化的输出被常量钉死,作为 T2~T5 的等价性基准(AC-01)。

- [ ] **T2 建注册表骨架** — `src/lib/ingest/adapters/{types,registry,opencode,claude,openclaw,hermes}.ts` + `test/framework-adapter-registry.test.ts`
  ...
  - **设计细节**:`InvokedSkill` 从 `interaction-utils`(叶子)导入,不从 `data-service`,免得把 DB 依赖拖进适配层;`resolveFrameworkId` 遇到不认识的框架**原样返回**,不塌缩成 'unknown';claude descriptor 为 `id:'claude', aliases:['claudecode'], platform:'claude'`,hermes 仅登记 descriptor 占位。
  - **不许做**:adapter 内访问 DB/网络;删或改被挂的原函数。
  - **怎么算过**:别名命中同一对象(`getAdapter('claudecode')===getAdapter('claude')`)、adapter 输出 == golden、引用相等三项断言通过(AC-01/AC-04)。
...
- [ ] **T3 dispatcher 改走注册表** — `data-service.ts:476`
  ...
  - **怎么算过**:golden/registry 两测试全绿;三框架结果不变、未知框架仍返回 null(AC-01/AC-03)。
...
- [ ] **T4 三处调用点收敛** — `upload/route.ts:167,292`、`rejudge/route.ts:61`、`proxy/end/route.ts:82,138,311`
  ...
  - **怎么算过**:rejudge 现在能抽出 openclaw 的 skill 了(AC-02);upload/proxy 行为不变(AC-01)。
  - **不许做**:不改那三个被挂函数的实现体。
...
- [ ] **T5 claude 归一化切 data-service 一处** — `data-service.ts:1556/1566/1575`
  ...
  - **怎么算过**:claude/claudecode 入库归一化结果不退化(AC-04)。
