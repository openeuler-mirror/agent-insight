...
├─ [D] 执行主机、模型、Case
├─ [E] 官方测试契约覆盖情况
└─ [F] 固定 Official Harness

实验详情
├─ [G] 五阶段进度
├─ [H] Resolve Rate、Official Harness 结果
├─ [I] 同基线趋势
└─ [J] Case 列表
...
| F | Official Harness 已作为 Benchmark 固定评估器绑定 | 前端固定展示 |
...
- `reuse-template`：锁定数据集 hash、Case 和 Official Harness，允许重新选择 OpenCode Agent、客户端和模型。
...
5. Step 4 固定展示并选中 Official Harness；
