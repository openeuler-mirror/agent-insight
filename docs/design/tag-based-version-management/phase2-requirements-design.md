...
| D-002 | Trace ↔ 用户标签绑定使用 `ExecutionTag` 关系表 | 业务标签筛选和版本聚合是核心路径，需要可索引查询；JSON 扫描会拖慢列表和聚合 |
