...
状态：步骤 04～07 后端已实现并通过真实 HTTP API 测试；步骤 01～07 已使用本地 SWE-bench Verified 真实 Case 串联验证；真实 OpenCode generate-only Smoke 已通过。

## 1. 结论

执行器不另起一套 Agent 执行实现，而是在现有 `agent-insight-client` 常驻进程中增加 HTTP 接收层和 Benchmark 编排层：

```text
POST /api/v1/benchmark-executions
  → 校验平台签名、requestDigest、TaskEnvelope
  → 持久化 runId 后返回 202
...
同一个 systemd/launchd 服务继续负责心跳、能力上报、WSS/long-poll 指令和故障注入；Benchmark HTTP 任务与原有任务共享一个本地执行槽。第一阶段并发固定为 1。

## 2. 高保真调用顺序

| 步骤 | 调用 | 执行器处理 | 成功后状态 |
|-|-|-|-|
| 04 | Agent Insight `POST {executorBaseUrl}/api/v1/benchmark-executions` | 鉴权、双端协议校验、按 `runId + requestDigest` 持久化 | 返回 `202 accepted`，后台开始运行 |
...
| 现有能力 | 处理方式 |
|-|-|
| `scripts/reliability-client.cjs` 常驻进程、安装、保活、心跳、能力发现 | 直接复用；HTTP listener 与现有循环同进程启动 |
...
| `handledCommands` 内存去重 | 不能复用；HTTP 任务必须使用磁盘状态保证重启后幂等 |
| 当前固定 `workspaceBase` 作为 Agent cwd | 改为每个 `runId` 独立的 prepared workspace |
| 原命令回执 `COMMAND_STATUS` | Benchmark 不复用；改为 progress、Artifact 和 complete 三类平台 API |
...
packages/benchmark-protocol/
  src/
    ...
    executor-contracts.ts           # HMAC token、执行请求、进度与终态 DTO
  schemas/
    agent-task.schema.json

services/executor/
  src/
    index.cjs                       # HTTP、Runner、通用能力注册表和声明式执行计划
...
test/
  benchmark-executor-api.test.ts    # 04～07 与 01～07 的真实 HTTP 验收
```
...
执行器监听地址和平台可达的 `executorBaseUrl` 写入现有客户端配置。安装器增加显式配置项，不根据多网卡 IP 自动猜测对外地址；未配置 `executorBaseUrl` 时不上报 Benchmark 能力，也不启动对外 listener。
...
不返回执行器业务版本。源 commit 等只作为部署诊断事实。
...
- 同 `runId + digest`：返回 `202` 和当前状态，不重复运行；
- 同 `runId` 不同 digest：`409 RUN_ID_CONFLICT`，永久失败；
- 执行槽被其他任务占用且本地没有接收该 Run：`409 SERVICE_BUSY`，平台稍后重发；
- Schema、能力或策略不支持：`422 TASK_SCHEMA_INVALID/CAPABILITY_MISSING/POLICY_UNSUPPORTED`；
- token、clientId、摘要或 callback origin 不匹配：`401/403`。

Agent Insight Scheduler 已按响应错误码分类：只有 `RUN_ID_CONFLICT` 永久失败；`SERVICE_BUSY` 保持同一 `runId + requestDigest`，延迟后重发。

### 8.3 平台回调

请求中的 `callbackBaseUrl` 是当前 Run 资源：`{insightOrigin}/api/benchmark/v1/runs/{runId}`。执行器只能调用 `${callbackBaseUrl}/progress` 和 `${callbackBaseUrl}/complete`；Artifact 固定上传到 `{insightOrigin}/api/benchmark/v1/artifacts`。执行器必须校验 origin 与本机配置的 `insightBaseUrl` 一致、路径中的 `runId` 与请求一致，不能将其当作任意 URL。所有回调复用现有 `deviceCredential + x-agent-insight-client-id`，平台再校验 Run 确实属于该 `clientId`。
...
不新增一套明文平台密钥。服务端使用当前有效 `ReliabilityClientCredential.credentialHash` 作为 HMAC key；执行器由本地 `deviceCredential` 计算同一 SHA-256 key。执行任务 token 包含 `aud/clientId/runId/requestDigest/expiresAt`，health token 使用独立 `purpose=health` 且不携带 Run。实现已移除全局 `AGENT_INSIGHT_BENCHMARK_DISPATCH_SECRET`。
...
3. 实现 `FileRunStore`、按客户端 HMAC、HTTP `/health` 和 execution 接口；
...
7. 更新 client bundle 和安装器，使守护进程上报真实 `executorBaseUrl` 与能力；
8. 补 Agent Insight 的 Artifact、progress、complete 接口，再进行端到端联调。

已完成的自动化测试直接启动临时 HTTP listener，而不只调用内部方法：

- 04～07：HMAC 鉴权、`202`、磁盘幂等、digest 冲突、busy、health、真实 Git Patch 收集、Artifact/progress/complete 回调和工作区清理；
- 01～07：从本地 Verified Parquet 通过官方 loader 读取 500 个真实 Case，选取真实 Case 经创建实验、启动调度、执行器、Artifact 和 complete 全部 HTTP 边界到达 `submitted`；
