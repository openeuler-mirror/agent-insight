...
> 关联项目：agent-insight
> 复杂度评估：**Medium**（P0 含 protobuf 解码，复杂度偏该档上限）
> base_commit：5976cbb（master）
> 变更类型：新增能力（feature）
> 状态：**v0.3（基于已评审的 openclaw-otel-adapter v0.2 重生成，按当前代码现状刷新）**
>
> **v0.3 关键说明（与历史版本的差异）**：本文件是对此前已评审通过（81/100）的 `openclaw-otel-adapter` v0.2 的**重生成 + 现状刷新**。需求层（场景/业务规则/FR/NFR/AC/TC）**实质不变**，但 §5.2 关键现状已据当前 master（5976cbb）核实更新——**同批两条目标架构已落地**：`framework-adapter-registry`（adapter 查表已生效）与 `otel-spool-consumer`（traces 端点已退薄壳、聚合落在 `src/lib/ingest/claude-otel/`）。因此本需求的服务端净新增进一步收敛。
>
> 参考材料：
> - **上游调研摘要（已吸收进本文件）** —— OpenClaw 面向可观测性存在三条已验证路径：A. OpenRouter Broadcast（零代码，但只覆盖经 OpenRouter 的 LLM 调用，不覆盖工具/skill/harness，且依赖 Langfuse/OpenRouter 生态，因此不纳入 agent-insight）；B. 原生 OTel 导出（OpenClaw 内置 `http/protobuf` 导出，支持纯配置与 exporter 插件两种形态，可覆盖模型调用、工具执行、skill 使用与生命周期 span，本需求迁移并落地的就是该路径）；C. 自定义插件 + Ingestion API（可按会话轮次重建数据，但依赖 Langfuse 私有 Ingestion 协议，不符合本平台统一走 OTLP 的边界，因此不纳入本需求）。本目录中的 Phase1/2/3 已将方案 B 的关键事实、约束与风险内嵌，无需依赖外部备份文档。
> - [`hermes-otel-adapter`](../hermes-otel-adapter/) —— 同类先行设计，本需求的服务端架构与其对齐并复用其适配层设计。
> - 已落地目标架构：[`otel-spool-consumer`](../otel-spool-consumer/)（薄壳端点 + 后台消费者，**已落地于 `src/lib/ingest/claude-otel/`**）、[`framework-adapter-registry`](../framework-adapter-registry/)（adapter 查表，**已落地于 `src/lib/ingest/adapters/`**，禁 per-framework 裸分支）。
...
需求价值：agent-insight 的核心定位是「框架无关」的 Agent 观测/评测/Skill 优化工程底座。OpenClaw 当前仅有**本地文件 watcher** 接入（chokidar 监听 `~/.openclaw/agents/**/*.jsonl`，要求 OpenClaw 与平台**同机部署**），无法服务远程/容器化/多实例的 OpenClaw 部署形态。新增标准 OTel/OTLP 接入可解除同机限制、统一接入协议，并兑现「多平台标准化接入」的产品承诺。

需求描述：使运行在 **OpenClaw**（自治 AI Agent 网关，模型无关，Signal/Telegram/Discord 等渠道接入）上的 Agent，能够通过**标准 OpenTelemetry（OTel/OTLP）协议**将运行链路数据上报到 agent-insight，被正确解析、归并为会话并在观测看板呈现，框架标识为 `openclaw`。本需求**是**「为 OpenClaw 新增一条标准协议的远程接入通路」，**不是**替代既有 watcher 接入（两者并存、接入指引互斥），也**不**改变 OpenCode/Claude 及 hermes 的接入行为。
...
>
> **关键协议缺口（P0 解决）**：OpenClaw 内置导出仅走 `http/protobuf`（忽略 gRPC），而平台 traces 端点当前仅受理 `application/json`（protobuf 返回 415，已核实 `traces/route.ts:22-27`）。**服务端必须新增 OTLP `http/protobuf` 解码**（落在 hermes 设计 NFR-004 预留的「仅新增解码适配、不动核心解析/归并」扩展点上），否则「纯配置」主路径不成立。
...
|Why|既有 watcher 接入要求同机部署、依赖 OpenClaw 私有 jsonl 格式；标准 OTLP 接入解除部署限制、降低格式耦合，并复用已落地的 spool/registry 标准化接入架构|
|Where|OpenClaw 运行环境（纯配置 env 指向平台，或安装 exporter 插件）；agent-insight 自托管服务端（Next.js + Prisma/SQLite）|
|How Much|**客户端**：主路径零代码（仅配置环境变量）；增强路径复用开源 aliyun openclaw-exporter（配 endpoint/鉴权头指向平台），自研插件仅作兜底规约。**服务端**：复用既有 OTLP 通路（已退薄壳 + spool）与 hermes 适配层设计，净新增 = protobuf 解码适配 + openclaw 语义映射条目 + 在已注册的 `adapters/openclaw.ts` 上扩展 OTel 整形能力|
...
|BR-005|新增 openclaw OTel 适配不得破坏现有 OpenCode/Claude/hermes 的接入解析行为，不得破坏既有 **watcher-openclaw** 链路（向后兼容）|存量接入不可回退；多线须共存|全部既有接入链路|
...
|BR-011|平台服务端对 openclaw 的解析/聚合必须复用已落地的统一接收/转换分层（接收与处理解耦的薄壳+spool、按框架查表转换的 registry），禁止为 openclaw 新增独立的接收路径或框架专属裸分支；新增 protobuf 编码支持不得改变既有框架（json 路径）的解析结果（具体落点见 §5.2 目标架构条目，模块级细节归 Phase2）|接收/转换分层已落地，须共用而非另起一套，避免冲突与返工|服务端架构、接收路径、转换层|
|BR-012|接入指引必须包含「同一 OpenClaw 实例的 watcher 与 OTel 上报互斥（只开一路）」的强制声明；平台对违反互斥导致的同会话双路重复不承诺去重|两路 framework 同值、会话键来源不同，强去重复杂度高且收益低（已确认不做强保证）|接入引导、文档、数据质量|
...
|编号|类别|名称|描述|优先级|
|-|-|-|-|-|
|NFR-001|兼容性|存量接入零回退|openclaw OTel 适配上线后，OpenCode/Claude/hermes 的接入解析、**watcher-openclaw** 链路的行为保持不变（既有用例全部通过）|P0|
...
|NFR-005|可维护性|新增框架低成本（registry 已落地的核验样板）|openclaw OTel 适配按已落地目标架构核验：≈ 新增 1 组语义映射条目 + 1 个整形适配 + 在已注册的 `adapters/openclaw.ts` 上扩展能力 + 编码层 protobuf 解码（框架无关），**0 处既有框架分支改动**（registry 的 `getAdapter` 查表已生效，无需新增 dispatcher 裸分支）|P1|
...
|AC-007|BR-005, NFR-001|兼容|存量接入不回退|openclaw OTel 上线后，OpenCode/Claude/hermes 既有接入回归用例全部通过；watcher-openclaw 链路行为不变|
...
|AC-016|FR-007, FR-012, BR-010, DC-009|接入|客户端接入规约完备|接入规约文档覆盖：纯配置 env 规约、插件复用配置规约（含版本兼容矩阵说明）、自研插件兜底规约；并记录两种路径实际发出的属性约定，服务端映射对这些约定有覆盖或降级保留；**且 DC-009 鉴权头假设若证伪，规约明确落实其二选一确定性走向（平台兼容 `Authorization: Basic` 头 / 切换自研兜底规约）**|
...
|TC-007|AC-007|OpenCode/Claude/hermes 既有用例 + watcher-openclaw 用例|执行存量回归用例|全部通过，无行为差异|
...
|openclaw OTLP 接入能力|服务端解析/归并/标识对 openclaw 的支持（复用 hermes 适配层设计 + 已落地 registry adapter + 语义映射条目）|
...
```text
你使用 skills aet-analysis-and-design，参考 docs/design/hermes-otel-adapter 的需求设计，
帮我生成针对 openclaw 开源框架的需求设计，输出到 docs/design/openclaw-adapter。
注意：你首先要深入调查 openclaw 对 otel 的支持；最终目标是：openclaw 中 agent 的执行记录
对接适配到 agent-insight 项目中。
```

#### 5.1.2 澄清

```text
执行模式确认：用户选择「基于已评审设计重生成」+「全流程 Phase1→Phase2→Phase3（各阶段经独立 Subagent 评审）」。
发现：此前已有完整且评审通过的 openclaw-otel-adapter v0.2 设计（在当前未提交变更中被删除、并以空骨架 openclaw-adapter 取代）。
据用户选择，以该 v0.2 为权威输入重生成到 openclaw-adapter，并按当前 master（5976cbb）现状刷新。

继承自 v0.2 的关键澄清（仍然有效）：
1. 客户端形态 = 双路径：纯配置内置 OTLP exporter（主） + aliyun 形态 exporter 插件（增强），文档两者都写；自研插件仅兜底。
2. 与既有 watcher 关系 = 并存 + 互斥指引；不做双路去重（已知限制）。
3. 服务端架构 = 复用已落地的 spool/registry 接收-转换分层 + hermes 适配层设计；openclaw 仅扩展映射/整形条目。
4. 范围深度 = 对齐 hermes 全量（观测链路 + 会话归并 + 子 Agent 树 + skill 一等抽取 + 自动注册 + 评测承接）。
5. protobuf 协议缺口 = P0 服务端补 http/protobuf 解码（落 hermes NFR-004 预留扩展点）。
6. 难度评估 = Medium。
```

### 5.2 关键现状（代码与材料依据，便于后续设计追溯；**v0.3 据 master 5976cbb 核实**）
...
- **平台 OTLP 端点已退薄壳（spool-consumer 已落地）**：`src/app/api/ingest/otel/v1/traces/route.ts` 现状为「解析 key→`normalizeClaudeOtlpTraces`→`appendOtelTraceEvents` 写 spool→返回 `{status:'accepted'}`」，**请求内不再同步落库**；聚合/落库在后台消费者管线 `src/lib/ingest/claude-otel/`（`aggregator.ts`/`traces-aggregator.ts`/`otlp-json.ts`/`spool.ts`/`types.ts`）。**这是 v0.2 假设「未落地」、v0.3 已落地的关键变化**——openclaw 的语义映射/整形将插入此聚合管线，而非 route 内联。
- **protobuf 仍未支持**：`traces/route.ts:22-27` 对 `application/x-protobuf` 仍返回 415，错误文案提示改用 http/json。**FR-002 的 P0 缺口真实存在、未被填补**。
- **会话归并键与 framework 来源**：归并键优先级 `session.id → service.instance.id → traceId`；`framework` 取自 OTLP `service.name`（归一化在 `claude-otel/otlp-json.ts`）。
- **framework-adapter-registry 已落地并完全接线**：`src/lib/ingest/adapters/`（`registry.ts`/`types.ts` + `opencode/claude/openclaw/hermes.ts`）。`getAdapter`/`listFrameworks`/`resolveFrameworkId` 已可用；skill 抽取调度 `extractInvokedSkillsFromSessionInteractions`（`src/lib/storage/data-service.ts:650-653`）已改为 `getAdapter(framework).extractSkills?.(normalized) ?? null`，**无 per-framework 裸分支**；`openclawAdapter`（`adapters/openclaw.ts`）已注册，`extractSkills` 指向既有 `extractSkillsWithVersionsFromOpenClawSession`，descriptor `{onboard:'plugin', platform:'openclaw'}`。`FrameworkAdapter` 已含 `normalizeForStorage?` 与 `extractSkills?` 扩展点；descriptor 的 `onboard` 仍为**单值**（`plugin|env|watcher`），**双模式（watcher+otel）需 `onboardModes?` 扩展，仍为协商项**。
...
- **上游调研结论已内化（替代外部备份引用）**：A. OpenRouter Broadcast 仅能观测经 OpenRouter 的 LLM 调用，无法覆盖 OpenClaw 的工具/skill/harness 全链路，且依赖 Langfuse/OpenRouter 专属能力，因此明确排除；B. 原生 OTel 导出是唯一同时满足“标准协议、平台统一接收、可覆盖全链路 span”的主路径，且要兼容“纯配置”和“插件增强”两种客户端形态；C. 自定义插件 + Ingestion API 虽能按会话轮次重建 trace，但协议是 Langfuse 私有 Ingestion，不并入本平台 OTLP 方案，仅作为调研背景保留，不进入需求/设计/开发范围。
- **子 Agent 树解门点（v0.3 行号刷新）**：`saveExecutionRecord` 中 `deriveSubagentExecutions` 的调用受 `targetRecord.framework === 'opencode'` 门限保护（**当前 `src/lib/storage/data-service.ts:2368`**，调用在 `:2370`；v0.2 锚点 `:2155` 已漂移）。`src/lib/storage/data-service.ts` 的 `deriveSubagentExecutions`（`:2437`）/`sweepStaleSubagents`（`:2601`）/`src/lib/engine/observability/agent-trace.ts` 的 `buildAgentCallTree`/`src/lib/engine/observability/agent-registration.ts` 均为冻结复用项。**行号漂移本身提示：实现期须按当时 master 重新定位锚点**。
- **接入引导现状**：`src/app/api/ingest/setup/auto/route.ts` 已含 openclaw 的 **watcher 模式**（框架枚举 `:109`、watcher 安装逻辑 `:185+/:246/:358`，并经公共别名路由 `/api/setup/openclaw-watcher` 下发 watcher client）；交互式 `src/app/api/ingest/setup/route.ts` **不含 openclaw**（已核实，grep 无匹配）。FR-006 需在两处补 openclaw 的 OTel 模式。
- **已知风险与限制（langfuse 报告 §6）**：
  ...
  - **插件鉴权头假设（DC-009，待验证）**：aliyun exporter 是否支持自定义 `x-witty-api-key` 头未经证实；验证失败走 DC-009 列明的两条确定性路径之一。
  - watcher/OTel 双开导致同会话双路重复呈现（S-017/BR-012）：互斥指引覆盖，平台不做强去重。
- **framework 标识对齐（已核实）**：`openclaw-parser.ts:160` 写入 `framework: 'openclaw'`，与 OTel 路径 `service.name=openclaw` 同值，满足 BR-001 单一口径。
...
| **v0.3** | **基于 v0.2 重生成 + 现状刷新（master 5976cbb）**：①目录迁至 `openclaw-adapter`、关联路径更新；②§5.2 据当前代码核实——`framework-adapter-registry` 与 `otel-spool-consumer` **已落地**（端点退薄壳、聚合在 `claude-otel/`、dispatcher 走 `getAdapter` 无裸分支、`openclawAdapter` 已注册）；③protobuf 仍 415（P0 缺口未变）；④子 Agent 解门锚点刷新为 `data-service.ts:2368`；⑤NFR-005 措辞改为「registry 已落地的核验样板」；⑥setup 现状刷新（auto 有 watcher、交互式缺 openclaw）；⑦补入上游三路径调研摘要，移除对本地备份文档的依赖。需求层（FR/NFR/AC/TC/场景/规则）实质不变 |
