...
采集侧额外覆盖 Hook 与 OTel 的批量 JSONL、合并标签、截断输出和延迟事件重复上报；修复后上述三条轨迹的平台工具调用数与物理执行数一致，适配器定向测试 34/34 通过。受控输入、运行时原始证据、凭据和模型配置不进入提交；脱敏页面截图仅在专题 README 中展示。

## 可视化验收证据

完整页面截图统一在 [专题 README 的“完整效果截图”章节](README.md#完整效果截图) 展示，本报告不重复嵌入图片。
