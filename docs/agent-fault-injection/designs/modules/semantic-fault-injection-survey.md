...
>
> 关联文档: 各故障模式设计文档（如 [analysis-paralysis](../../../agent-ras/designs/features/analysis-paralysis.md)、[domain-cognitive-bias](../features/domain-cognitive-bias.md) 等）
...
|------|------|----------------------|
| **故障分类体系** | MAS-FIRE 的 15 种故障 | 提供系统化故障目录，各具体故障模式可归类为其中子类 |
| **Semantic-Level vs Structure-Level 注入区分** | MAS-FIRE | 语义级故障应聚焦 Semantic-Level 注入 |
| **Prompt Modification 注入机制** | MAS-FIRE + AutoTransform | 通过 System Prompt 注入角色定义偏差 |
| **Interception & Response Rewriting** | MAS-FIRE + AutoInject | 拦截 Agent 输出并在 reasoning chain 中注入偏差内容 |
| **O, L, S 过程级评估** | MAS-FIRE | O: 是否检测到故障 / L: 能否恢复 / S: 恢复后能否正确完成 |
| **Challenger 防御机制** | AutoTransform/AutoInject | 让其他 Agent 挑战可疑输出 |
| **Inspector 防御机制** | AutoTransform/AutoInject | 引入专门的监察 Agent, 检测到异常时介入并强制终止 |

### 5.2 评估框架的设计启示

从 MAS-FIRE 的 (O, L, S) 三元组推导出各故障场景的专用评估指标:

```mermaid
flowchart TD
    RS_OT["系统级: 鲁棒性<br/>RS = 注入后仍能正确完成的任务 / 基准任务数"]

    O_OT["O (检测率)<br/>= N_trigger / N_total<br/><br/>系统是否意识到异常?<br/>高: 能检测到故障<br/>低: 静默陷入故障, 无人察觉"]

    L_OT["L (中止率)<br/>= N_fixed / N_trigger<br/><br/>检测到故障后, 是否能成功中止?<br/>高: 有效的 break-loop 机制<br/>低: 明知异常但无法停止"]

    S_OT["S (恢复后任务成功率)<br/>= N_final_success / N_trigger<br/><br/>中止异常后, 能否正确完成任务?<br/>L - S 的 gap 揭示<br/>异常期间丢失了哪些关键信息"]

    E_OT["E (效率损失)<br/>= (Token_fault - Token_baseline) / Token_baseline<br/>量化故障导致的 Token 消耗增加比例"]

    C_OT["C (纠正成本)<br/>= Token_recovery / Token_excess<br/>恢复到正确路径的 Token / 故障浪费的 Token"]
