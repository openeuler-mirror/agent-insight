...
- 本机 Worker 提交 `collect-result` → `persistFiCollectIngress`（写入）→ `finishFiJudgeFromDb`（只读 Prisma 再 Judge）
- FI markers → `src/lib/fault-injection/trace-markers.ts`（贴近 RasTraceMarker）
- Trace API / Run 页读已落库 `Session.interactions` + `FaultInjectionRun.markersJson` + 可选真 RAS（只读拉取）
...
服务端**不**直接打开本机 Worker 产物目录。`collect-result` POST body **仅作写入入口**；**FI Run / Judge / rejudge 一律只读 Prisma**（`Session` + `FaultInjectionRun`），不以上传 body 作为评判/展示真源。
