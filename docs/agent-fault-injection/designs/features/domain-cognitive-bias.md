...
> 文档类型：Phase1 需求分析 + 业界调研 + 故障场景说明书（合并） | 关联项目：agent-insight / agent-fault-injection  
> 复杂度：**Medium–High**（检测依赖外部证据对齐；子类多、边界需与幻觉区分）  
> 关联文档：
> - [规划错误 / Planning Error（RAS 检测规划）](../../../agent-ras/designs/features/planning-error.md)
> - [语义层故障注入调研](../agent-semantic-fault-injection-survey.md)
...
| 规划错误（Planning Error） | 策略层不可行 / 忽略约束 | ⚠️ 方案中（[planning-error](../../../agent-ras/designs/features/planning-error.md)） |
...
- 为故障注入评测页提供可执行剧本（对齐 [MAS-FIRE / AutoInject 调研](../agent-semantic-fault-injection-survey.md)）。
...
> 开放场景很少有「正确答案」作 oracle。评测真值来自**注入剧本标签**（`fault_type` + `injection_point` + `expected_detection`），而不是领域金标准答案。通用语义注入机制深拆见 [agent-semantic-fault-injection-survey.md](../agent-semantic-fault-injection-survey.md)。
...
13. **Jia et al.** "MAS-FIRE: Fault Injection and Reliability Evaluation for LLM-Based Multi-Agent Systems." arXiv:2602.19843, 2026. 数据：https://github.com/wxhhxn/MASFIRE；本仓拆解见 [agent-semantic-fault-injection-survey.md](../agent-semantic-fault-injection-survey.md)  
...
18. [planning-error.md](../../../agent-ras/designs/features/planning-error.md)（含同构的故障注入章节）  
19. [AgentDebug 错误词表](../../../../skills/agent-debug-diagnosis/references/02-error-taxonomy.md)  
20. [语义层故障注入调研](../agent-semantic-fault-injection-survey.md)
---
