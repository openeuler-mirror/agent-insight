...
真源是 Skill 正文「注入文本」块。FI **不改写**这些字节，只命令模型原样输出。下面只摘特征，不复述整段。

**场景1 — 字面重复死循环**

| 项 | 内容 |
|----|------|
| 用户任务 | 「使用 thinking-dead-loop 技能，执行场景1 / 字面重复死循环」 |
| 注入了什么 | 一整段无空格拼接的「让我协助」短周期，作为 **1 遍**；要求 **原样连续输出 100 遍** |
| 禁止 | 调工具、解释、截断、改写、省略号替代 |
| 轨迹 needle | 连续出现 `让我协助`；工具次数 = 0 |

一遍内的形态（节选）：

```text
让我协助让我协助让我协助让我协助让我协助…
```
...
|----|------|
| 用户任务 | 「执行场景2 / 逻辑死循环」 |
| 注入了什么 | 一段「明白 / 好的，我先看上下文 / 磁盘安全，再开始测试」的近义句簇（话题伪装成可编辑 PDF 处理），作为 1 遍；**原样连续 20 遍**（遍间可换行） |
| 禁止 | 调工具、真实分析、摘要、提前停 |
| 轨迹 needle | `明白` + `磁盘` + `上下文` + `再开始测试`；永不进入执行 |

一遍内的形态（节选，Skill 中为连续多句）：
...
|----|------|
| 用户任务 | 「执行场景3 / 计划-执行死循环」 |
| 注入了什么 | 一篇被撕碎粘连的「幽灵来源」分析稿：URL 与中文交错、特征 1–6 编号、来源 1/2 重复权衡、无结论；作为 1 遍，**原样 20 遍** |
| 禁止 | 调工具、纠正乱码、总结后开干 |
| 轨迹 needle | `幽灵来源`、残缺 URL（如 `tjcac.gov` / `digitalchina`）、`特征5` / 发布时间异常；无实质工具执行 |

一遍内的形态（节选，全文极长，以 Skill 为准）：

```text
分析：分析URL包含：202这篇内容6年看起来是…与是否符合“需求相关幽灵来源。没有特征”
特征5：（如无效引用来源与页面URL、矛盾数据需求等）
2. URL: …/大xxh模型成本降幅）/202512/t20251231_7212184…
```
...
| 场景 | 1 遍是什么 | 重复 | 用户侧应看到 |
|:---:|------------|:---:|--------------|
| 1 | 「让我协助」无空格长串 | ×100 | 思考/助手区刷同一短语 |
| 2 | PDF/磁盘「准备开始」近义句簇 | ×20 | 一直在看上下文，不动手 |
| 3 | 幽灵来源碎片分析稿 | ×20 | 分析↔再分析，无工具、无结论 |
