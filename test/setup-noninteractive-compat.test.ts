...
const ALL_FRAMEWORKS = 'opencode,openclaw,claude,codeagent,hermes,jiuwen,qoder,actrail';
...
test('Bash 恢复 URL、CLI 和环境变量非交互入口，并保留全部框架', async () => {
  ...
  for (const flag of ['OPENCODE', 'OPENCLAW', 'CLAUDE', 'CODEAGENT', 'HERMES', 'JIUWEN', 'QODER', 'ACTRAIL']) {
