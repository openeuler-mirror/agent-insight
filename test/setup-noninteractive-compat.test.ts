...
const ALL_FRAMEWORKS = 'opencode,openclaw,claude,codeagent,hermes,jiuwen';
...
test('Bash 恢复 URL、CLI 和环境变量非交互入口，并保留全部框架', async () => {
  ...
  assert.match(script, /^NONINTERACTIVE_FRAMEWORKS="opencode,openclaw,claude,codeagent,hermes,jiuwen"$/m);
  ...
  for (const flag of ['OPENCODE', 'OPENCLAW', 'CLAUDE', 'CODEAGENT', 'HERMES', 'JIUWEN']) {
