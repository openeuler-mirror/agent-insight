...
test('inactivity applies to every framework without an assistant answer at exactly ten minutes', () => {
  ...
  for (const framework of ['actrail', 'opencode', 'claudecode', 'hermes', 'openclaw', 'jiuwenswarm', 'langfuse-langgraph', 'direct_llm', 'codex', 'pi-agent', 'qoder', 'new-agent', undefined]) {
...
test('Goal Plus terminal failures retain their status and reason with parsed or serialized failures', () => {
  const failure = [{ failure_type: 'goal_plus_pi_session_failed' }];
  for (const failures of [failure, JSON.stringify(failure)]) {
    const result = getTraceLifecycle(new Date(now - 3600_000), { framework: 'pi-agent', failures, lastIngestedAt: received }, now);
    assert.equal(result.traceStatus, 'failed');
    assert.equal(result.traceStatusReason, 'goal-plus-session-failed');
  }
  assert.equal(getTraceLifecycle(new Date(now), { failures: 'invalid json' }, now).traceStatus, 'success');
});
