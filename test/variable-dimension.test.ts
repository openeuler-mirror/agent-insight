// 变量维度策略测试：原生 Trace 比较、共享条件与维度注册。
// 覆盖：extractValue 取正确字段 / controlledFields 返回维度正确集 /
// framework 保留 stub；其他已支持的维度按 Trace 事实分组。
...
test('SKILL_DIMENSION.extractValue 读 trace.skill', () => {
  ...
  assert.equal(SKILL_DIMENSION.extractValue({ skill: null }), '__NONE__');
  assert.equal(SKILL_DIMENSION.extractValue({}), '__NONE__');
});

// ─── controlledFields ───────────────────────────────────────────────────────

test('LLM_DIMENSION.controlledFields 返回 agentName/skill/skillVersion/framework', () => {
  const fields = LLM_DIMENSION.controlledFields().map((f) => f.field);
  assert.deepEqual(fields, ['agentName', 'skill', 'skillVersion', 'framework']);
});
...
test('SKILL_DIMENSION.controlledFields 变 skill 时控制 agentName+model+framework', () => {
  const fields = SKILL_DIMENSION.controlledFields().map((f) => f.field);
  assert.deepEqual(fields, ['agentName', 'model', 'framework']);
});
...
test('Skill 版本是变量的一部分，无 Skill 明确表示基线', () => {
 assert.equal(SKILL_DIMENSION.extractValue({skill:'review',skillVersion:2}),'review@v2');
 assert.equal(SKILL_DIMENSION.extractValue({skill:null}),'__NONE__');
});
...
test('Agent 对比能以 Agent 名称为变量，评估器对比有独立策略', () => {
  assert.equal(getDimension('agent')?.extractValue({agentName:'agent-b'}),'agent-b');
  assert.equal(getDimension('evaluator')?.dimension,'evaluator');
});
