...
test('同名碰撞：name-scan 会选到缺脚本的旧目录；storageDirOverride 绕过它选到正确目录', () => {
  ...
  const origDataDir = process.env.AGENT_INSIGHT_HOME;
  ...
  try {
    ...
    process.env.AGENT_INSIGHT_HOME = cwd;
    process.chdir(cwd);
    ...
    if (origDataDir === undefined) delete process.env.AGENT_INSIGHT_HOME;
    else process.env.AGENT_INSIGHT_HOME = origDataDir;
