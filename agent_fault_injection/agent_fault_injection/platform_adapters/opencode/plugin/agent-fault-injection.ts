...
function applyToolResultRewrite(
  ...
  for (const step of plan) {
    ...
    const when =
      step.when && typeof step.when === "object"
        ...
        : undefined
    if (!whenMatches(when, tool, callIndex)) {
      continue
    }
    const args =
      step.args && typeof step.args === "object"
        ...
        : {}
    if (op === "tool_result.replace_text" || op === "tool_result.replace_all") {
      ...
      return {
        output: output.split(source).join(dest),
        meta: {
          applied: true,
          op,
          tool,
...
/** Mirror Python apply_system_rewrite. */
function applySystemRewrite(
  plan: Array<Record<string, unknown>>,
  systemParts: string[],
  ...
  for (const step of plan) {
    ...
    if (op === "system.replace_text") {
      ...
      const rewritten = joined.split(source).join(dest)
      return {
  ...
  return { parts, meta: { applied: false, op: null } }
}
...
function buildInjectedMessage(role: string, text: string): Record<string, unknown> {
  // Support both {role, content} and OpenCode {info, parts} shapes.
  return {
    role,
    content: text,
...
/** Mirror Python apply_messages_rewrite. */
function applyMessagesRewrite(
  plan: Array<Record<string, unknown>>,
  messages: unknown[],
): { messages: unknown[]; meta: Record<string, unknown> } {
  ...
  for (const step of plan) {
    ...
    if (op === "messages.history.drop") {
      ...
      for (let index = current.length - 1; index >= 0; index -= 1) {
        const role = messageRole(current[index]).toLowerCase()
        if (role === "system") {
          continue
        }
        dropIdx.push(index)
        if (dropIdx.length >= count) {
          break
        }
      }
      if (!dropIdx.length) {
    ...
    if (op === "messages.inject") {
      ...
      if (
        ...
        if (Array.isArray(updated.parts)) {
        ...
        } else if (Array.isArray(updated.blocks)) {
          updated.blocks = [
            { type: "text", text: prefix },
            ...updated.blocks,
          ]
          ...
          updated.content = prefix
          updated.parts = [{ type: "text", text: prefix }]
        }
        current = [
      ...
      const message = buildInjectedMessage(role.trim(), text)
  ...
  return { messages: current, meta: { applied: false, op: null } }
}

/** Mirror Python apply_assistant_text_rewrite. */
function applyAssistantTextRewrite(
  plan: Array<Record<string, unknown>>,
  text: string,
  callIndex: number,
  ...
  for (const step of plan) {
    ...
    if (op === "assistant.truncate") {
      ...
      if (typeof maxChars !== "number" || !Number.isInteger(maxChars)) {
        continue
      }
    ...
    if (op === "assistant.replace_text") {
      ...
      return {
        text: text.split(source).join(dest),
        meta: {
          applied: true,
          op,
          kind: "assistant",
